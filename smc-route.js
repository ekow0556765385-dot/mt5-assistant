...
/* ── ONE SERVICE SENDS ─────────────────────────────────────────────
   Production runs TWO services from this codebase — nurturing-magic and
   mt5-assistant — and `lastConfluence` above lives only in this
   process's memory. So each instance dedupes perfectly against itself
   and neither knows the other exists: one EA cycle produces one alert
   per running service, identical in every value, which is exactly the
   duplicate being reported. No in-process dedupe can fix that, because
   the two processes are each behaving correctly.
   So sending gets ONE owner, the same rule this platform applies
   everywhere else. Set TELEGRAM_ALERTS=off on every service except the
   one the EA actually posts to.
   Default is ON, so an unset variable changes nothing — a silent
   platform would be a worse failure than a duplicated one. */
const ALERTS_ON = String(process.env.TELEGRAM_ALERTS || 'on').toLowerCase() !== 'off';

/* Who am I? Printed on every send so the duplicate can be confirmed from
   the logs rather than inferred: two lines for one confluence, with two
   different names, is the proof. */
const SERVICE = process.env.RAILWAY_SERVICE_NAME || process.env.SERVICE_NAME || 'unnamed-service';
if (!ALERTS_ON) {
  console.log(`[CONFLUENCE] ${SERVICE}: TELEGRAM_ALERTS=off — detecting and serving /confluence as normal, not sending.`);
} else {
  console.log(`[CONFLUENCE] ${SERVICE}: sending Telegram alerts. If another service is also sending, subscribers get duplicates.`);
}
...
function detectConfluence(userId, scope, symbol, tf) {
  ...
  for (const cand of candidates) {
    ...
    const matchingOB = cand.matchingOB, matchingFVG = cand.matchingFVG;
    {
      ...
      /* NORMALISED. `pattern.timeframe` is set on some entry paths and
         undefined on others, so the same confluence arriving through
         /smc/patterns and through ingestPatterns produced two different
         keys and both sent. Falling back to the payload's timeframe
         makes the key identical whichever door it came through. */
      const ckTf = String(pattern.timeframe || tfLabel || 'H1').toUpperCase();
      const confluenceKey = `${pattern.name}|${ckTf}|${direction}`;
      ...
      /* Detection, storage and /confluence carry on regardless — the
         panel must keep working on every service. Only the SEND is
         owned by one of them. */
      if (ALERTS_ON) {
        console.log(`[CONFLUENCE] ${SERVICE} sending ${direction} ${pattern.name} ${symbol} ${pattern.timeframe||tfLabel}`);
        sendTelegram(confluenceMsg);
        sendTelegramToUser(userId, confluenceMsg, symbol);
      } else {
        console.log(`[CONFLUENCE] ${SERVICE} suppressed ${direction} ${pattern.name} ${symbol} (alerts off on this service)`);
      }
    }
  }
  return newAlerts;
