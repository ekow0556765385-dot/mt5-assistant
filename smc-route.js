...
const { getChatIdForUser, getWatchPairsForUser } = require('./telegram-store');
...
// Restart storm guard — same problem the pattern alerts had.
// lastConfluence lives only in memory, so every restart wipes it and the
// next EA payload looks like a brand-new confluence on every pair,
// firing the whole book at once. A deploy, a crash loop or a changed env
// var all do this. Fix: the FIRST sighting of a given key after boot is
// recorded silently. A genuinely new confluence still alerts seconds
// later; the stale backlog does not.
