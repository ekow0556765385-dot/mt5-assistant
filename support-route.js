...
function customerRouter(requireAuth) {
  ...
  r.post('/api/support/threads/:id/resolve', async (req, res) => {
    try {
      ...
      await system(t.id, 'Marked as solved by the customer');
      await patch('support_threads', `id=eq.${t.id}`, { stage: 'resolved', resolved_at: now, resolved_by: 'customer',
  ...
  r.post('/api/support/threads/:id/rate', async (req, res) => {
    try {
      ...
      await system(t.id, rating === 'yes' ? 'Customer says this solved it 👍' : 'Customer says this did not solve it 👎');
      await patch('support_threads', `id=eq.${t.id}`, { rating, rated_at: new Date().toISOString(), unread_admin: true });
      res.json({ thread: await loadThread(t.id, false) });
  ...
  r.post('/api/support/threads/:id/reopen', async (req, res) => {
    try {
      ...
      const now = new Date().toISOString();
      await system(t.id, 'Reopened by the customer');
      await patch('support_threads', `id=eq.${t.id}`, { stage: 'review', resolved_at: null, resolved_by: null,
...
function adminRouter({ audit, licenceSharing }) {
  ...
  r.get('/api/support', async (req, res) => {
    try {
      ...
      const list = await rows('support_threads',
        `select=id,user_id,topic,subject,stage,unread_admin,last_message_at,rating,waiting_since,reminded_at,resolved_by${q}` +
  ...
  r.post('/api/support/:id/stage', async (req, res) => {
    try {
      ...
      await system(t.id, `Moved to ${stageName(t.topic, stage)}`);
...
module.exports = {
  customerRouter, adminRouter, start, cleanUp, staleSweep, planOf, licenceOf, stageName,
  // exported for tests
