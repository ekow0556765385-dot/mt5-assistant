...
function pressureOf(ctx) {
  ...
  /* A reversal building on the candle that has NOT closed yet. It is real
     information — the trader can see it on the chart — but it can still
     unwind before the close, so it carries less weight than a closed one
     and always says it is unfinished. */
  ...
  /* The candle that has not closed yet. Worth less than a closed pattern
     because it can still unwind, and it says so — but a reversal building
     against an open trade is exactly what the trader needs to hear NOW. */
  if (forming) add('forming', 'Reversal forming on this candle', P.FORMING_AGAINST,
    `${forming.name} at ${patConf(forming)}% — this candle has not closed, so it may not finish this way.`);
