...
const CONDITIONS = {
  ...
  spread(w) {
    ...
    if (rel <= 1.2) return res(PASS, `Spread ${pts} points, normal for this session.`, 'broker feed', metric);
    const wide = `Spread ${pts} points against a median of ${medPts} — ${Math.round((rel - 1) * 100)}% wide${costBit}.`;
