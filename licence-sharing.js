...
async function setState(userId, action) {
  ...
  if (action === 'clear') {
    ...
    patch = {
      sharing_state: null,
      // NOT 'manual'. The sweep never touches a manually-decided licence,
      // so marking a clear as manual meant a customer who later really did
      // share their key would never be warned or paused automatically —
      // the score would climb and nothing would happen. The accepted
      // machines and holders below already score zero, so the sweep leaves
      // this setup alone by itself, and acts on anything NEW as normal.
