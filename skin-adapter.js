...
function renderPatFilter(){
  ...
  const sym=normalisePair(typeof activePair!=='undefined'?activePair:'');
  if(patFilter==='All'){
    host.style.display='none'; mine.style.display='';
    const dbs=(typeof dataBySymbol!=='undefined'&&dataBySymbol[sym])||{};
    const rows=[];
    ['H1','H4'].forEach(tf=>{
    ...
    mine.innerHTML = rows.length ? rows.map(p=>{
      ...
      return '<div class="pattern-row"><div style="flex:1">'+
        ...
        (conf>=70?'alertable':'below the 70% alert threshold')+'</div></div>'+
        '<span class="pconf" style="color:'+(conf>=70?'var(--text)':'var(--muted)')+'">'+conf+'%</span></div>';
    }).join('') : '<div class="rr-empty">No patterns detected on '+sym+' yet.</div>';
    return rows.length;
  }
  /* H1 / H4 — the host already renders exactly one timeframe, so switch
     the chart timeframe and show its list. */
  mine.style.display='none'; host.style.display='';
  if(typeof activeChartTF!=='undefined' && activeChartTF!==patFilter){
    const b=$('tf'+patFilter);
    if(b && typeof switchChartTF==='function') switchChartTF(patFilter,b);
  }
  return host.querySelectorAll('.pattern-row').length;
}
