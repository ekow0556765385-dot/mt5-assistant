...
document.querySelectorAll('.pair-row').forEach(r=>{
  r.addEventListener('click',()=>{ setTimeout(()=>{ nudgeChart(); drawRSI(); paintWatchlist(); paintVerdict(); },80); });
});

/* ── loop ────────────────────────────────────────────────────────── */
decorateWatchlist();
function tick(){
  try{ drawTape(); drawRSI(); paintWatchlist(); paintNow(); paintVerdict(); paintMiniAlerts(); }
