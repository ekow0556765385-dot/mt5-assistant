...
const RISK = {
  ...
  // News pause — prop firm compliant (FTMO, The Funded Trader, MyForexFunds etc)
  // No NEW trades within this window around high impact news events
  newsPauseMinsBeforeEvent: 5,   // pause 5 minutes before event
  newsPauseMinsAfterEvent:  5,   // pause 5 minutes after event

  // Which currencies to watch — pause if the active pair contains any of these
  // e.g. GBPUSD is paused on GBP or USD news. XAUUSD paused on USD news.
  newsCurrencies: ['USD', 'EUR', 'GBP', 'JPY', 'CHF', 'AUD', 'CAD', 'NZD'],

  // Map pairs to their relevant currencies for news filtering
  pairCurrencies: {
...
// ── News pause checker ───────────────────────────────────────────────────────
// Returns { paused: true, reason: '...' } if within news window
// newsEvents comes from appState.newsEvents (populated by ForexFactory feed in app.js)
function checkNewsPause(symbol, newsEvents) {
  ...
  for (const event of newsEvents) {
    ...
    if (secsToEvent > 0 && secsToEvent <= pauseBefore) {
      ...
      return {
        paused: true,
        reason: `News pause — ${event.title} (${eventCurr}) in ${minsLeft} min. No new entries within ${RISK.newsPauseMinsBeforeEvent} min of high impact news.`,
        event:  event.title,
        currency: eventCurr,
    ...
    if (secsSinceEvent >= 0 && secsSinceEvent <= pauseAfter) {
      ...
      return {
        paused: true,
        reason: `News pause — ${event.title} (${eventCurr}) released ${minsAgo} min ago. Waiting ${minsLeft} more min for market to stabilise.`,
        event:  event.title,
        currency: eventCurr,
        minsAgo,
        minsLeft
...
// ── Persistence — save and load session state across restarts ────────────────
function saveSession() {
  try {
    const data = {
      ...
      decisionLog:        agentState.decisionLog.slice(0, 50), // save last 50
...
function loadSession() {
  try {
    ...
    // Check if saved session is from today — if not, reset trade count
    ...
    if (sameDay) {
      ...
      // Restore enabled/demoMode state
...
function riskGate(decision, appState) {
  ...
  // ── News pause check (prop firm compliant) ──────────────────────────────────
  if (action === 'BUY' || action === 'SELL') {
    const newsCheck = checkNewsPause(symbol, appState.newsEvents);
    if (newsCheck.paused) {
      return { pass: false, reason: newsCheck.reason };
    }
  }
...
function registerAgentRoutes(app, getAppState, smcStore, candlesStore) {
  ...
  app.post('/api/agent-orders/confirm', (req, res) => {
    ...
    saveSession(); // persist after confirmed execution
    res.json({ ok: true });
  });
  ...
  app.post('/api/agent-session-reset', (req, res) => {
    ...
    saveSession(); // persist reset state
...
Agent ready for new trades`);
  ...
  app.post('/api/agent-control', (req, res) => {
    ...
    saveSession(); // persist mode settings
...
function startAgentLoop(getAppState, smcStore, candlesStore) {
  ...
  // Auto-save session every 5 minutes as backup
