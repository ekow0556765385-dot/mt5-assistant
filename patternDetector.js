...
function detectPatterns(candles, timeframe) {
  ...
  // Measured over a FIXED window of the most recent bars, not the whole
  // array. Feeds send different history lengths (50 on /api/update, 60 on
  // /api/multi-candles), and averaging over "whatever arrived" gave the same
  // candle different size floors on different feeds — a Doji on one, an
  // Inside Bar on the other. Every feed now measures the same bars.
  const FLOOR_WINDOW = 50;
  let rangeSum = 0, rangeCount = 0;
  for (let k = Math.max(0, n - FLOOR_WINDOW); k < n; k++) {
