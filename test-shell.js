...
(async () => {
  ...
  await check('another module can open it by postMessage', async () => {
    ...
    const active = Array.from(doc.querySelectorAll('.nav-item.active')).map(b => b.dataset.path);
    assert.deepStrictEqual(active, ['/arbiter'], 'exactly one highlight, on Arbiter: ' + active);
  });

  await check('the same fix works for every module, not just Arbiter', async () => {
    for (const [mod, path] of [['brain', '/brain'], ['smc', '/smc-panel'], ['patterns', '/patterns'], ['math', '/math']]) {
      win.dispatchEvent(new win.MessageEvent('message', { data: { bw: 'open', module: mod }, origin: 'https://app.test' }));
      await wait(120);
      const active = Array.from(doc.querySelectorAll('.nav-item.active')).map(b => b.dataset.path);
      assert.deepStrictEqual(active, [path], mod + ' -> ' + active);
    }
  });
