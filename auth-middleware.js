...
async function getUserPlan(userId) {
  try {
    const { data } = await axios.get(
      `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${userId}&select=plan,plan_key,status,expires_at,licence_key,access_status,access_reason,grace_until`,
...
function accessState(sub) {
  ...
  if (sub.plan === 'pro' && sub.expires_at) {
    ...
    if (expired) {
      const graceEnd = sub.grace_until ? new Date(sub.grace_until)
...
async function validateKey(req, res) {
  ...
  try {
    const { data } = await axios.get(
      ...
      `&select=plan,status,expires_at,user_id,bound_account,access_status,access_reason,grace_until`,
      { headers }
    );
