...
function requirePlan(minPlan) {
  return async (req, res, next) => {
    ...
    // ── Set a session cookie for this origin ──────────────────────
    // The dashboard forwards a single-use ticket (or, as a fallback,
    // the raw token) into this page's iframe src. That only proves
    // identity for THIS page load — it's dead afterward (tickets are
    // one-time) or absent from later same-origin fetch() calls the
    // page makes on its own (e.g. Trading Brain's /api/analyse).
    // Setting the real token as an httpOnly cookie here means every
    // subsequent same-origin request from inside the iframe carries
    // it automatically, without the frontend needing to know or
...
module.exports = { requireAuth, requirePlan, getMe, validateKey, regenerateKey, getUserFromToken, getUserPlan, isOwner, issueTicketRoute, SUPABASE_URL, SUPABASE_SERVICE, supabaseServiceHeaders, htmlRedirect, MONTHLY_CREDIT_USD, getUserCredits, deductCredits, resetUserCredits };
