...
// The columns every caller relies on, and the newer ones read alongside.
// Kept apart for one reason: if the newer columns are missing — the
// support-inbox migration not yet run — asking for them makes Supabase
// reject the WHOLE query, and the catch below would then quietly return
// plan 'free' for every customer: every paying account downgraded, the
// dashboard locked, and nothing in the logs but one warning. Instead,
// that one failure retries without the newer columns.
const PLAN_COLUMNS = 'plan,plan_key,status,expires_at,licence_key,access_status,access_reason,grace_until,licence_expires_at,pending_plan,pending_plan_key,pending_plan_at,sharing_state,sharing_accounts';
const PLAN_COLUMNS_NEWER = 'restore_notice,restore_notice_at';
  ...
  try {
    ...
    try {
      ({ data } = await axios.get(
        `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${userId}&select=${PLAN_COLUMNS},${PLAN_COLUMNS_NEWER}`,
        { headers: supabaseServiceHeaders() }
      ));
    } catch (e) {
      if (!/restore_notice/.test(JSON.stringify(e.response?.data || e.message))) throw e;
      console.warn('[AUTH] restore_notice columns missing — run migration-support-inbox.sql. Reading without them.');
      ({ data } = await axios.get(
        `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${userId}&select=${PLAN_COLUMNS}`,
        { headers: supabaseServiceHeaders() }
      ));
    }
...
function denySharingBlocked(req, res) {
  ...
  if (req.accepts && req.accepts('html') && !req.path.startsWith('/api')) {
    return res.status(403).send(
      ...
      // Reworded for the new scoring: account count is no longer what is
      // scored, so "used on several MetaTrader accounts" was no longer the
      // reason. It says what kind of thing was seen, not how it is measured.
      ...
      'Your licence key looks to be in use by more than one trader, so the Pro ' +
      'dashboard is paused while we take a look.</p>' +
      '<p style="font-size:14px;line-height:1.65;color:#eceae0;margin:0 0 18px">' +
      'Your EA, indicators and Telegram alerts are still running, so any open ' +
      'trades stay covered.</p>' +
      '<p style="font-size:14px;line-height:1.65;color:#9a9890;margin:0 0 22px">' +
      'Your account page explains what to do if there is an explanation.</p>' +
  ...
  return res.status(403).json({
    error: 'Dashboard access is paused while your licence is reviewed. Your EA, indicators and alerts are still running. See your account page.',
...
async function getMe(req, res) {
  ...
  res.json({
    ...
    // Why access is withdrawn, when it is — the text you typed when banning
    // or suspending. accessState() already carries it, and the licence
    // refusal the EA prints in MT5 already shows it; without this the
    // account page, the one place a banned trader is told to go, could
    // not say why.
    access_reason:   graceInfo.reason || null,
    // The same support address the emails use (SUPPORT_EMAIL on Railway),
    // so the account page can offer a real way to reply. Null when unset,
    // and the page then points to the website instead.
    support_email:   (process.env.SUPPORT_EMAIL || '').trim() || null,
    // "Your review is complete" — set when access is restored from the
    // Inbox, shown on every pane until the customer opens the Pro
    // Dashboard, which clears it.
    restore_notice:  sub?.restore_notice || null,
...
async function validateKey(req, res) {
  const { key, account, fp, holder, tzMin } = req.body;
  if (!key) return res.status(400).json({ valid: false, error: 'No key provided' });

  const headers = supabaseServiceHeaders();
  // Set once the subscription is found. The recorder needs it to look up
  // the Blackwood name, so the broker holder name can be reduced to the
  // right surname. Every record() call happens after it is set.
  let subUserId = null;
  const record = (outcome) => licenceAttempts.recordAttempt({
    supabaseUrl: SUPABASE_URL, headers, licenceKey: key, account, outcome, req,
    fp, holder, tzMin, userId: subUserId
  });
...
async function getUserCredits(userId) {
  try {
    ...
    /* AN ERROR MUST NOT LOOK LIKE A FULL WALLET.
       All three failure paths here used to return MONTHLY_CREDIT_USD, so
       a missing row, a NULL column or a thrown request all rendered as a
       brand-new $8.00/$8.00 balance. That is why the real figure flashes
       on load and is then replaced: the first paint is the true value and
       the second is this fallback overwriting it.
       `known` distinguishes "the balance is 8.00" from "we could not read
       the balance", so the caller can decide rather than being handed a
       number that was invented here. */
    if (data && data.length > 0) {
      const raw = data[0].credit_balance;
      if (raw != null) {
        return { balance: parseFloat(raw), resetAt: data[0].credit_reset_at, known: true };
      }
      /* Row exists but the column was never populated — a real state for
         an account created before credits existed. Treat it as a fresh
         cycle, but say that is what happened. */
      console.warn('[CREDITS] credit_balance is NULL for user=' + userId + ' — treating as a new cycle');
      return { balance: MONTHLY_CREDIT_USD, resetAt: data[0].credit_reset_at, known: true, seeded: true };
    }
    console.warn('[CREDITS] no subscriptions row for user=' + userId);
    return { balance: null, resetAt: null, known: false };
  } catch (e) {
...
async function deductCredits(userId, amount) {
  ...
  /* Never write a balance derived from a number we could not read — that
     would turn a transient read failure into a permanent full wallet. */
  if (!current.known || current.balance == null) {
    console.error('[CREDITS] refusing to deduct against an unknown balance for user=' + userId);
    return null;
  }
...
module.exports = {
  ...
  /* accessState already knows what grace, suspension and expiry mean.
     The daily credit sweep was making that judgement for itself and
     getting it wrong, so it is exported now and there is ONE definition
     of "is this account entitled to a new cycle". */
  accessState,
  /* Exported for Arbiter's automatic reports: a scheduled report must pass
     the SAME entitlement test as a request, not a copy of it. Additive —
     nothing that already imports from this file changes. */
  planRank, sharingBlocked };
