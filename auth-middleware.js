...
// CHANGE (account binding): the EA now sends `account` (the MT5 trading
// account login) alongside the key. The first account to validate with a
// given key "claims" it — bound_account gets set on that row. Any later
// validation attempt from a *different* account number is rejected, which
// is what actually stops a shared/leaked key from running on someone
// else's terminal, instead of relying on the ToS alone.
//
// `account` is optional so older EA builds (pre-account-binding) still
// validate exactly as before — they just don't get this protection until
// the subscriber updates.
//
// Requires a `bound_account` column (int8/bigint, nullable) on the
...
async function validateKey(req, res) {
  ...
  try {
    const { data } = await axios.get(
      `${SUPABASE_URL}/rest/v1/subscriptions?licence_key=eq.${key}&select=plan,status,expires_at,user_id,bound_account`,
      { headers: supabaseServiceHeaders() }
    );
    ...
    if (account) {
      if (sub.bound_account == null) {
        // First terminal to validate with this key — claim it.
        try {
          await axios.patch(
            `${SUPABASE_URL}/rest/v1/subscriptions?licence_key=eq.${key}`,
            { bound_account: account, updated_at: new Date().toISOString() },
            { headers: supabaseServiceHeaders({ 'Content-Type': 'application/json' }) }
          );
          console.log(`[LICENCE] Key bound to account=${account} user=${sub.user_id}`);
        } catch (e) {
          // A binding write failure shouldn't block a legitimate first-time
          // validation — log it and let this call through; the next
          // validation attempt will just try to bind again.
          console.error('[LICENCE] Failed to bind account:', e.response?.data || e.message);
        }
      } else if (String(sub.bound_account) !== String(account)) {
        console.warn(`[LICENCE] Rejected — key already bound to account=${sub.bound_account}, attempted from account=${account}, user=${sub.user_id}`);
        return res.json({ valid: false, error: 'Licence key already active on a different trading account. Regenerate your key from the dashboard to reset it, or contact support.' });
      }
    }
