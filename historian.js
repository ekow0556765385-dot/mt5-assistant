...
function create(opts){
  ...
  function testLevels(cd, tol){
    ...
    S.levels.forEach(function(lv){
      ...
      if(heldIt) lv.holds++; else lv.breaks++;
    });
  }
  ...
  function observe(cd, read){
    ...
    /* equal levels are decisive for a range and worth the same treatment */
    marked.filter(q=>/EQ/.test(q.label||'')).slice(-2).forEach(function(q){
      trackLevel(q.p, q.label==='EQ-H'?'equal-high':'equal-low', cd, tol); });
  ...
  function reset(){ S.bars=0; S.eras=[]; S.levels=[]; S.trend=null; S.lastRegime=null; S.claims={}; }
