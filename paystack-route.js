...
const SITE_URL        = process.env.SITE_URL        || 'https://comforting-strudel-6fb70f.netlify.app';
...
router.post('/api/pay/initiate', async (req, res) => {
  const { planKey, email, userId, currency } = req.body;

  if (!planKey || !email || !userId) {
    return res.status(400).json({ error: 'planKey, email and userId are required' });
  }
...
router.post('/api/pay/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.event === 'charge.success') {
    ...
    try {
      ...
      // TODO: Send email with licence key (Step 5)
      // sendWelcomeEmail(tx.customer.email, result.licenceKey, RAILWAY_URL + '/dashboard');
