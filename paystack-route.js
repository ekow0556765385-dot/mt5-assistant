...
async function updateSubscription(userId, planKey, paystackRef, legalMeta, card) {
  ...
  try {
    ...
    if (row && row.paystack_ref === paystackRef && row.licence_key) {
      ...
      return { licenceKey: row.licence_key, expiresAt: row.expires_at, plan: row.plan, planKey, wasRenewal: true };
    }
    // Kept for the renewal case below: a different ref, same customer.
    if (row && row.licence_key) existingKey = row.licence_key;
  } catch (e) {
    console.warn('[PAYSTACK] Idempotency check failed, proceeding anyway:', e.response?.data || e.message);
  }

  // REUSE the existing key. A renewal is the same customer continuing, so
  // minting a new key here silently broke their setup: the EA and all four
  // indicators hold the key in an input field, so the moment this row was
  // overwritten every terminal started failing validation until they found
  // the new key and re-pasted it everywhere. A yearly subscriber would hit
  // that once a year, right after paying.
  //
  // Matches renewLicence() for Lifetime and the Software Licence, which
  // both say renewal restores the existing key rather than issuing a new
  // one. A fresh key is minted only when the row genuinely has none — a
  // first purchase, or a row whose key was cleared by a downgrade to free.
  // Deliberate reissues go through regenerateKey() in auth-middleware.js,
  // which also clears bound_account; that is the only path that should
  // ever replace a working key.
  const licenceKey = existingKey || generateLicenceKey();
  if (existingKey) {
    console.log(`[PAYSTACK] Reusing existing licence key for user=${userId} (renewal)`);
  }
  ...
  // First MT5 licence term. Lifetime's one-time fee includes 12 months on
  // the compiled EA/indicators (Software Licence s.4); after that it is
  // $100/year. Stamped HERE, at purchase, so no Lifetime row is ever
  // created without a term — a NULL licence_expires_at means "allow
  // forever" in auth-middleware, so forgetting this would silently switch
  // enforcement off for every future customer.
  //
  // Pro is left NULL on purpose: Pro pays for the key through the
  // subscription and has no separate term.
  const licenceExpiresAt = planConfig.plan === 'lifetime'
    ? (() => { const d = new Date(); d.setUTCMonth(d.getUTCMonth() + 12); return d.toISOString(); })()
    : null;
  ...
  const payload = {
    ...
    // A grace period belongs to the cycle it was granted for. Once that
    // cycle is paid, it is spent — so clear it here.
    //
    // Leaving it set was silently destructive: accessState() prefers
    // grace_until over the automatic GRACE_DAYS window whenever the column
    // holds a value, so one goodwill extension in March would still be the
    // "grace end date" in April. The next genuinely failed payment locked
    // that customer out instantly with no 3-day window, while everyone else
    // got one, and nothing anywhere showed why.
    grace_until:      null,
    // Paying cancels any scheduled downgrade. Someone who queued a drop to
    // free and then renewed has plainly changed their mind, and leaving the
    // schedule armed would quietly downgrade them after they just paid.
    pending_plan:     null,
  ...
  // PAIR ENTITLEMENT ON UPGRADE. A free user picks 3 pairs, so watch_pairs
  // holds 3 entries. /api/watch-pairs returns `watch_pairs.slice(0, limit)`
  // and the limit becoming 6 does NOT lengthen a 3-item array — the
  // fall-back to all six only fires when the column is EMPTY, which it is
  // not once they have used the free picker. So without this line an
  // upgraded customer keeps receiving alerts on 3 pairs, silently, forever.
  // The upgrade is the event, so the entitlement moves here rather than
  // waiting for the user to discover a picker and re-choose.
  if (planConfig.plan === 'pro' || planConfig.plan === 'lifetime') {
    // Same list as BW_ALL_PAIRS in app.js / telegram-store.js, which
    // mirrors the EA's WatchPairs default. Keep all three in step.
    payload.watch_pairs = ['EURUSD','GBPUSD','USDJPY','XAUUSD','USDCHF','GBPJPY'];
  }
  ...
  // Only written for Lifetime, and only on a genuine first purchase.
  // A repeat charge on an existing Lifetime row must not reset a term the
  // customer has already renewed — that is renewLicence()'s job.
  if (licenceExpiresAt) payload.licence_expires_at = licenceExpiresAt;
  ...
  // existingKey is only set when the row already had a key under a DIFFERENT
  // paystack_ref — i.e. the same customer paying again. That is the cleanest
  // signal available here for "renewal, not first purchase".
  return { licenceKey, expiresAt, plan: planConfig.plan, planKey, licenceExpiresAt,
           wasRenewal: !!existingKey };
}
...
async function renewLicence(userId, paystackRef, planKey, legalMeta) {
  ...
  await axios.patch(
    `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${userId}`,
    {
      ...
      updated_at:          new Date().toISOString(),
      // Which version of the Software Licence this term was agreed under.
      ...(legalMeta && legalMeta.agreedAt
        ? { legal_agreed_at: legalMeta.agreedAt, legal_version: legalMeta.version || LEGAL_VERSION }
        : {})
    },
    { headers }
  );
...
router.post('/api/pay/initiate', requireAuth, async (req, res) => {
  ...
  // Still required for renewals. A renewal starts a fresh 12-month term
  // under whatever the Software Licence says today, so the agreement has
  // to be captured again rather than inherited from the original
  // purchase — the terms may have changed since (LEGAL_VERSION tracks
  // this). Only the wording differs.
  if (agreedToLegal !== true) {
    return res.status(400).json({
      error: isRenewal(planKey)
        ? 'You must agree to the Software Licence before renewing.'
        : 'You must agree to the Terms of Use, Privacy Policy, Software Licence, and Risk Disclaimer before continuing.'
    });
  }
  ...
  // Renewals are only meaningful for Lifetime holders. Check BEFORE
  // creating the Paystack transaction: if this only failed later in
  // renewLicence(), the customer would have already been charged for
  // something we then refuse to deliver, and you would be issuing a
  // manual refund. Fail before any money moves.
  if (isRenewal(planKey)) {
    try {
      const { data } = await axios.get(
        `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${userId}&select=plan,licence_key`,
        { headers: { Authorization: `Bearer ${SUPABASE_SVC}`, apikey: SUPABASE_SVC } }
      );
      const row = data && data[0];
      if (!row || row.plan !== 'lifetime') {
        return res.status(400).json({
          error: 'The $100 MT5 licence renewal applies to Lifetime plans only. ' +
                 'Your Pro subscription already includes the licence key at no extra cost.'
        });
      }
      if (!row.licence_key) {
        return res.status(400).json({
          error: 'No licence key found on this account. Contact support before renewing.'
        });
      }
    } catch (e) {
      console.error('[PAYSTACK] Renewal eligibility check failed:', e.response?.data || e.message);
      return res.status(500).json({ error: 'Could not verify your plan. Try again in a moment.' });
    }
  }
...
router.get('/api/pay/callback', async (req, res) => {
  ...
  try {
    ...
    if (isRenewal(planKey)) {
      const r = await renewLicence(userId, reference, planKey, legalMeta);
      console.log(`[PAYSTACK] Callback success — licence renewal for ${userId}`);
      return res.redirect(
        `${SITE_URL}/account?renewal=success&until=` + encodeURIComponent(r.licenceExpiresAt.slice(0, 10))
      );
    }

    const result = await updateSubscription(userId, planKey, reference, legalMeta, tx.authorization);
    console.log(`[PAYSTACK] Callback success — ${planKey} for ${userId}${result.wasRenewal ? ' (renewal)' : ''}`);

    // A RENEWAL goes back to the account page, not the marketing site. The
    // customer started this from Billing, so that is where they expect to land
    // — and the first-purchase banner is wrong for them anyway: it announces a
    // licence key they have had for a year and says "redirecting to your Pro
    // dashboard" to someone who was already using it.
    if (result.wasRenewal) {
      return res.redirect(
        `${SITE_URL}/account?renewed=success&plan=${encodeURIComponent(result.planKey || result.plan)}` +
        (result.expiresAt ? '&until=' + encodeURIComponent(String(result.expiresAt).slice(0, 10)) : '')
      );
    }

    // First purchase: the marketing site's welcome banner, which shows the new
    // licence key and hands them on to the dashboard.
...
router.post('/api/pay/webhook', async (req, res) => {
  ...
  if (event.event === 'charge.success') {
    ...
    if (isRenewal(planKey)) {
      try {
        const r = await renewLicence(userId, tx.reference, planKey, legalMeta);
    ...
    try {
      ...
      const custName  = tx.metadata?.custom_fields?.find(f => f.variable_name === 'name')?.value
                       ...
                       || '';
      // planKey, not plan: 'pro' alone cannot tell Monthly from Yearly, so
      // every Pro customer was getting identical copy about a renewal date
      // that is a month away for one of them and a year away for the other.
      sendWelcomeEmail(custEmail, custName, result.planKey || result.plan, result.licenceKey,
                       result.expiresAt, result.licenceExpiresAt)
        .then(r => {
