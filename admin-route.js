...
const { sendSharingNotice, sendAccessNotice } = require('./email-service');
...
// Inbox (Messages & Inbox). Mounted here, AFTER requireAdmin, so every
// route in it is behind your admin login like the rest of this file.
router.use(require('./support-route').adminRouter({ audit, licenceSharing }));
...
// Email AND last name per user, for the licence-sharing tab. A sibling
// of fetchEmails() rather than a change to it: /api/users depends on that
// function returning a plain id -> email map.
//
// The last name is the last word of the Blackwood full name — the same
// rule the licence check uses to pick the broker surname to compare, so
// the tab and the scorer read names the same way.
async function fetchPeople() {
  ...
  try {
    for (let page = 1; page <= 25; page++) {
      ...
      const list = data.users || data || [];
      list.forEach(u => {
        const words = String(u.user_metadata?.full_name || '').trim().split(/\s+/).filter(Boolean);
        map[u.id] = { email: u.email || null, lastName: words.length ? words[words.length - 1] : null };
      });
      if (list.length < 200) break;
    }
  } catch (e) {
...
// ── POST /admin/api/users/:id/sharing ──────────────────────────
// Apply or lift a licence-sharing strike by hand.
//   { action: 'warn' }   -> notice on their account page
//   { action: 'block' }  -> Pro dashboard closed (account, key, alerts stay)
//   { action: 'clear' }  -> withdraw, and remember the count it was cleared
//                           at so the nightly sweep does not re-apply it
//
// 'warn' and 'block' are recorded as manual, which the sweep never
// overrides in either direction — an operator decision outranks the
// heuristic until it is explicitly cleared.
router.post('/api/users/:id/sharing', async (req, res) => {
  const userId = req.params.id;
  const action = String(req.body?.action || '').toLowerCase();
  const reason = String(req.body?.reason || '').trim().slice(0, 500);
  if (!['warn', 'block', 'clear', 'unblock'].includes(action)) {
    return res.status(400).json({ error: "action must be 'warn', 'block', 'unblock' or 'clear'" });
  }
  // Pausing a paying customer's dashboard, or accepting a setup so it
  // never flags again, are decisions you will need to explain later.
  // A warning and a restore are not, so they stay one click.
  if ((action === 'block' || action === 'clear') && reason.length < 8) {
    return res.status(400).json({ error: 'Give a reason of a few words — it goes on the record' });
  }
  try {
    const before = await axios.get(
      `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${userId}&select=sharing_state,sharing_accounts`,
      { headers: svcHeaders() }
    );
    const patch = await licenceSharing.setState(userId, action);
    audit({ req, action: 'sharing.' + action, targetUser: userId,
            before: before.data?.[0], after: patch,
            note: reason ? `licence sharing ${action} — ${reason}` : `licence sharing ${action}` });
    res.json({ ok: true, state: patch.sharing_state || null });
  } catch (e) {
    console.error('[ADMIN] sharing change failed:', e.response?.data || e.message);
    res.status(502).json({ error: 'Could not change sharing state' });
  }
});

// ── POST /admin/api/users/:id/sharing-email ────────────────────
// Sends the licence-sharing notice by hand. Kept SEPARATE from the state
// change above on purpose: warning someone and emailing them are different
// decisions, and there are cases for each without the other (a state change
// while you investigate quietly; an email to a customer you have already
// spoken to). Combining them would remove that choice.
router.post('/api/users/:id/sharing-email', async (req, res) => {
  const userId = req.params.id;
  const stage  = String(req.body?.stage || 'warning').toLowerCase();
  if (!['warning', 'blocked'].includes(stage)) {
    return res.status(400).json({ error: "stage must be 'warning' or 'blocked'" });
  }
  try {
    const { data } = await axios.get(
      `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${userId}` +
      `&select=plan,sharing_accounts`,
      { headers: svcHeaders() }
    );
    const sub = (data && data[0]) || {};

    // The address lives on auth.users, not on subscriptions.
    let email = null, name = '';
    try {
      const { data: u } = await axios.get(
        `${SUPABASE_URL}/auth/v1/admin/users/${userId}`, { headers: svcHeaders() }
      );
      email = u?.email || null;
      name  = u?.user_metadata?.first_name || u?.user_metadata?.full_name || '';
    } catch (e) {
      console.warn('[ADMIN] sharing-email — could not read user:', e.response?.data?.message || e.message);
    }
    if (!email) return res.status(404).json({ error: 'No email address on file for this user' });

    const r = await sendSharingNotice(email, name, {
      stage, accounts: sub.sharing_accounts || 0, plan: sub.plan || null
    });
    if (!r.ok) return res.status(502).json({ error: r.error || 'Email failed to send' });

    audit({ req, action: 'sharing.email', targetUser: userId,
            after: { stage, to: email }, note: `sharing notice (${stage}) emailed` });
    res.json({ ok: true, to: email });
  } catch (e) {
    console.error('[ADMIN] sharing-email failed:', e.response?.data || e.message);
    res.status(502).json({ error: 'Could not send the notice' });
  }
});

// ── GET /admin/api/licence-sharing ─────────────────────────────
// Every Pro licence with its score, band, the evidence behind it, what
// has been done about it, and the history — everything the tab draws.
//
// The evidence is written by the nightly sweep (licence-sharing.js), so
// this reads it rather than recomputing. POST .../rescore runs the sweep
// now, for when you want fresh numbers without waiting for 00:20 UTC.
router.get('/api/licence-sharing', async (req, res) => {
  try {
    let subs;
    try {
      const { data } = await axios.get(
        `${SUPABASE_URL}/rest/v1/subscriptions?licence_key=not.is.null&plan=in.(pro,lifetime)` +
        `&select=user_id,licence_key,plan,access_status,access_reason,sharing_state,sharing_state_source,` +
        `sharing_score,sharing_band,sharing_evidence,sharing_updated_at,sharing_ok_fps,sharing_ok_holders`,
        { headers: svcHeaders(), timeout: 15000 }
      );
      subs = data || [];
    } catch (e) {
      // The new columns do not exist until migration-licence-sharing-v2
      // has run. Say so, rather than showing an empty tab that reads as
      // "nobody is sharing".
      if (/column .* does not exist|sharing_score|sharing_evidence/i.test(e.response?.data?.message || '')) {
        return res.json({ collecting: false, licences: [],
          message: 'Run migration-licence-sharing-v2.sql in Supabase, then Rescore now.' });
      }
      throw e;
    }

    const [people, policy] = await Promise.all([fetchPeople(), licenceSharing.readPolicy()]);

    // History: every sharing and access decision for these accounts, by
    // you, from the audit log. Automatic moves are added from the row
    // itself, since the sweep does not write to the admin audit log.
    const ids = subs.map(s => s.user_id);
    let auditRows = [];
    if (ids.length) {
      try {
        const { data } = await axios.get(
          `${SUPABASE_URL}/rest/v1/admin_audit?target_user=in.(${ids.join(',')})` +
          `&or=(action.like.sharing.*,action.like.access.*)&select=at,action,note,target_user&order=at.desc&limit=500`,
          { headers: svcHeaders(), timeout: 10000 }
        );
        auditRows = data || [];
      } catch { /* as above */ }
    }
    // Plain words, as in the prototype. The audit log stores action codes
    // ("sharing.unblock") and a note ("licence sharing unblock — reason");
    // showing those raw made the history a column of identical codes with
    // the reasons buried. The reason is the part after the dash.
    const LABEL = {
      'sharing.warn':    'Warning applied',
      'sharing.block':   'Pro dashboard paused',
      'sharing.unblock': 'Dashboard reopened',
      'sharing.clear':   'Cleared and accepted',
      'sharing.email':   'Notice emailed',
      'access.banned':   'Banned',
      'access.suspended':'Suspended',
      'access.active':   'Access restored',
      'access.warned':   'Account marked as warned'
    };
    const reasonOf = note => {
      const m = /\s—\s(.+)$/.exec(String(note || ''));
      return m ? m[1] : null;
    };
    const byUser = {};
    for (const r of auditRows) {
      let text = LABEL[r.action] || r.note || r.action;
      if (r.action === 'sharing.email') {
        text = /blocked/.test(r.note || '') ? 'Pause notice emailed' : 'Warning notice emailed';
      }
      // Bans and restores store the reason itself as the note; sharing
      // actions store "licence sharing X — reason". Read each its own way.
      const why = String(r.action).startsWith('access.') ? (r.note || null) : reasonOf(r.note);
      if (why && r.action !== 'sharing.email') text += ' — ' + why;
      (byUser[r.target_user] = byUser[r.target_user] || []).push({ at: r.at, by: 'you', text });
    }

    const counts = { clear: 0, watch: 0, review: 0, shared: 0, unscored: 0 };
    const licences = subs.map(s => {
      const ev = s.sharing_evidence || null;
      const band = s.sharing_band || null;
      if (band && counts[band] !== undefined) counts[band]++; else counts.unscored++;
      const history = (byUser[s.user_id] || []).slice();
      if (s.sharing_state_source === 'auto' && s.sharing_state) {
        history.push({ at: s.sharing_updated_at, by: 'automatic',
          text: (s.sharing_state === 'blocked' ? 'Pro dashboard paused' : 'Warning applied') +
                ` automatically — score ${s.sharing_score} (${band})` });
      }
      history.sort((x, y) => String(y.at).localeCompare(String(x.at)));
      const who = people[s.user_id] || {};
      return {
        userId: s.user_id,
        email: who.email || null,
        lastName: who.lastName || null,
        // Prefix only. The whole key is a credential, and nothing in this
        // tab needs more than enough to tell two keys apart.
        keyPrefix: String(s.licence_key).slice(0, 9),
        plan: s.plan,
        score: s.sharing_score ?? null,
        band,
        state: s.sharing_state || null,
        stateSource: s.sharing_state_source || null,
        access: { status: s.access_status || 'active', reason: s.access_reason || null },
        evidence: ev,
        // Accepted machines OR holders: a customer cleared while still on
        // an older EA has holders accepted but no machines recorded yet.
        accepted: (s.sharing_ok_fps || []).length > 0 || (s.sharing_ok_holders || []).length > 0,
        updatedAt: s.sharing_updated_at || null,
        history
      };
    }).sort((x, y) => (y.score ?? -1) - (x.score ?? -1));

    res.json({ collecting: true, policy, counts, licences });
  } catch (e) {
    console.error('[ADMIN] /api/licence-sharing failed:', e.response?.data || e.message);
    res.status(502).json({ error: 'Could not read licence sharing' });
  }
});

// ── POST /admin/api/licence-sharing/rescore ────────────────────
// Runs the nightly sweep now. Useful right after the migration, and
// after clearing someone, to see the new numbers straight away.
router.post('/api/licence-sharing/rescore', async (req, res) => {
  try {
    const r = await licenceSharing.sweep();
    audit({ req, action: 'sharing.rescore', after: r, note: 'licence sharing rescored by hand' });
    res.json({ ok: true, ...r });
  } catch (e) {
    console.error('[ADMIN] rescore failed:', e.message);
    res.status(502).json({ error: 'Rescore failed' });
  }
});

// ── GET/POST /admin/api/sharing-policy ─────────────────────────
// Whether Watch (warning) and Review (dashboard pause) act by
// themselves or wait for you. Likely shared is never automatic — a ban
// always needs you, so it has no switch here.
router.get('/api/sharing-policy', async (req, res) => {
  res.json(await licenceSharing.readPolicy());
});

router.post('/api/sharing-policy', async (req, res) => {
  const ok = v => v === 'auto' || v === 'manual';
  const { watch, review } = req.body || {};
  if (!ok(watch) || !ok(review)) {
    return res.status(400).json({ error: "watch and review must each be 'auto' or 'manual'" });
  }
  try {
    const before = await licenceSharing.readPolicy();
    await axios.post(`${SUPABASE_URL}/rest/v1/admin_settings`,
      { key: 'sharing_policy', value: { watch, review }, updated_at: new Date().toISOString() },
      { headers: { ...svcHeaders(), 'Content-Type': 'application/json',
                   Prefer: 'resolution=merge-duplicates,return=minimal' }, timeout: 8000 });
    audit({ req, action: 'sharing.policy', before, after: { watch, review },
            note: `sharing policy: watch ${watch}, review ${review}` });
    res.json({ ok: true, watch, review });
  } catch (e) {
    console.error('[ADMIN] sharing-policy failed:', e.response?.data || e.message);
    res.status(502).json({ error: 'Could not save the policy' });
  }
});
...
router.post('/api/users/:id/access', async (req, res) => {
  ...
  try {
    ...
    // Tell them by email when they are banned or suspended — only when that
    // is NEW, so re-saving a ban already in place does not email them
    // again. Nothing was ever emailed before: a customer banned by mistake
    // saw only their EA stop, with no idea why or where to appeal. Sent
    // after the ban is saved and never able to undo it.
    if ((status === 'banned' || status === 'suspended') && before[0].access_status !== status) {
      (async () => {
        try {
          const { data: u } = await axios.get(`${SUPABASE_URL}/auth/v1/admin/users/${userId}`,
            { headers: svcHeaders(), timeout: 6000 });
          if (!u || !u.email) return;
          const first = String(u.user_metadata?.full_name || '').trim().split(/\s+/)[0] || null;
          const r = await sendAccessNotice(u.email, first, { status, reason: reason ? String(reason).trim() : null });
          audit({ req, action: 'access.emailed', targetUser: userId,
                  note: r && r.ok ? `${status} email sent` : `${status} email FAILED — ${r && r.error || 'unknown'}` });
        } catch (e) { console.warn('[ADMIN] access email failed:', e.message); }
      })();
    }
