...
router.get('/api/licence-sharing', async (req, res) => {
  try {
    ...
    // Join the recorded attempts to the strike state on each subscription,
    // so the tab shows what has ALREADY been done about a key rather than
    // just the raw evidence. Failure here degrades to an empty map — the
    // evidence is still worth showing without it.
    let stateByKey = new Map();
    try {
      const { data: subs } = await axios.get(
        `${SUPABASE_URL}/rest/v1/subscriptions?licence_key=not.is.null` +
        `&select=user_id,licence_key,plan,sharing_state,sharing_state_source,sharing_accounts,sharing_updated_at`,
        { headers: svcHeaders() }
      );
      for (const r of subs || []) stateByKey.set(r.licence_key, r);
    } catch (e) {
      console.warn('[ADMIN] could not join sharing state:', e.response?.data?.message || e.message);
    }
    ...
    const flagged = [...byKey.entries()]
      .map(([key, rec]) => ({
        ...
        userId:        stateByKey.get(key)?.user_id || null,
        plan:          stateByKey.get(key)?.plan || null,
        sharingState:  stateByKey.get(key)?.sharing_state || null,
        sharingSource: stateByKey.get(key)?.sharing_state_source || null,
        confirmed:     stateByKey.get(key)?.sharing_accounts ?? null,
        stateUpdated:  stateByKey.get(key)?.sharing_updated_at || null
      }))
