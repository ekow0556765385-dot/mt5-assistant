...
// What plan is this user on? Used to label alerts so a free-tier message
// says plainly what it is and what it is not. Returns 'free' on any lookup
// failure: mislabelling a Pro alert as free is cosmetic, while promising a
// free user Pro-grade signals is a claim we cannot keep.
async function getPlanForUser(userId) {
  try {
    ...
    const row = data && data[0];
    return (row && row.plan) ? String(row.plan).toLowerCase() : 'free';
  } catch (e) {
    console.warn('[TELEGRAM-STORE] getPlanForUser failed:', e.response?.data?.message || e.message);
    return 'free';
  }
}
...
module.exports = { createLinkCode, linkChatIdToCode, getAllLinkedChatIds, getChatIdForUser,
                   getWatchPairsForUser, getPlanForUser, alertsEntitled, BW_ALL_PAIRS, BW_FREE_LIMIT };
