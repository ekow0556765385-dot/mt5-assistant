...
async function sendWelcomeEmail(toEmail, name, plan, licenceKey, expiresAt, licenceExpiresAt) {
  ...
  const html = buildWelcomeEmail(name, plan, licenceKey, expiresAt, licenceExpiresAt);
  const planName = PLAN_NAMES[plan] || plan;
...
// Sent by hand from the admin console, never automatically. The tone is
// deliberately measured: at this stage the most likely explanation is a
// customer running their own second terminal, not deliberate resale, and
// accusing a paying customer of theft over what turns out to be a demo
// account is far more costly than asking politely.
//
// stage: 'warning' (first notice, nothing restricted)
//        'blocked' (dashboard paused)
async function sendSharingNotice(toEmail, name, opts) {
  const { stage, accounts, plan } = opts || {};
  const isBlocked = stage === 'blocked';
  const n = Number(accounts) || 0;
  const seen = n > 1
    ? `Our records show this key validating from <strong style="color:#eceae0">${n} different MetaTrader account numbers</strong>.`
    : `Our records show this key validating from more than one MetaTrader account.`;
  ...
  const html = buildNoticeEmail({
    ...
    subhead:  isBlocked ? 'Licence key used on multiple accounts'
                        : 'This is a notice, not a restriction',
    paragraphs: [
      `Your Blackwood licence key is issued for one trader. ${seen}`,
      isBlocked
        ? `Because this has continued, access to the Pro dashboard has been paused. Your subscription, your licence key and your Telegram alerts are all still active, and nothing has been cancelled or charged.`
        : `If those are all your own terminals, there is nothing to worry about and no action is needed &mdash; we would simply ask that the key is not shared outside your own accounts. Nothing has been restricted.`,
      isBlocked
        ? `If there is a reasonable explanation &mdash; a broker change, a prop-firm account, a terminal you no longer use &mdash; reply to this email and we will lift it. We would much rather sort this out than lose you as a customer.`
        : `Sharing a licence key outside your own accounts is not permitted under the Software Licence, and repeated use across further accounts may lead to dashboard access being paused. Reply to this email if anything here looks wrong.`
    ],
    reassurance: isBlocked
      ? 'Your subscription and licence key remain active. This is a pause on dashboard access only, and it is reversible.'
