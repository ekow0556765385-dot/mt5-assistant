  ...
  ['daily', 'weekly'].forEach(kind => {
    app.get(`/api/arbiter/report/${kind}`, gate, async (req, res) => {
      try {
        ...
        const [facts, report] = await Promise.all([factsFor(req.user.id, kind, period), storedReport(req.user.id, kind, period.key)]);
    ...
    app.post(`/api/arbiter/report/${kind}`, gate, async (req, res) => {
      const uid = req.user.id;
      try {
        const period = periodFor(kind, req.body && req.body.date);
        const existing = await storedReport(uid, kind, period.key);
        if (existing) return res.json({ ok: true, period: period.key, report: existing, cached: true });

        const owner = !!(req.subscription && req.subscription.owner);
        if (!owner && getUserCredits) {
          const c = await getUserCredits(uid);
          if (!(c && c.balance > 0)) return res.status(402).json({ ok: false, error: 'Out of analysis credits for this cycle', resetAt: c && c.resetAt });
        }
        const facts = await factsFor(uid, kind, period);
        if (!facts.calls.total && !facts.trades.closed && !facts.habits.raised.length)
          return res.status(409).json({ ok: false, error: 'Nothing happened in this period to write about.' });
        ...
        const text = ((r.data && r.data.content) || []).map(c => c.text || '').join('').replace(/\*\*/g, '').trim();
        if (!text) return res.status(502).json({ ok: false, error: 'Claude returned nothing' });

        const u = (r.data && r.data.usage) || {};
        const cost = ((u.input_tokens || 0) / 1e6) * PRICE_IN + ((u.output_tokens || 0) / 1e6) * PRICE_OUT;
        let balance = null;
        if (!owner && deductCredits) balance = await deductCredits(uid, cost);

        const row = { user_id: uid, kind, period: period.key, facts, report_md: text, model: MODEL,
                      cost: +cost.toFixed(6), created_at: iso(now()) };
        let saved = row;
        try {
          const { data } = await http.post(T.reports, row, H({ 'Content-Type': 'application/json', Prefer: 'return=representation' }));
          saved = Array.isArray(data) && data[0] ? data[0] : row;
        } catch (e) {
          if (e.response && e.response.status === 409) saved = (await storedReport(uid, kind, period.key)) || row;
          else log.warn('[ARBITER] report save failed (report still returned):', e.message);
        }
        log.log && log.log(`[ARBITER] ${kind} report user=${uid} cost=$${cost.toFixed(4)}`);
        res.json({ ok: true, period: period.key, report: saved, cost, creditBalance: balance });
      } catch (e) {
        log.error('[ARBITER] report write failed:', e.response ? JSON.stringify(e.response.data) : e.message);
        res.status(500).json({ ok: false, error: 'Could not write the report' });
      }
    });
  });
...
module.exports.reports = { buildFacts, weekOf, dayRange, REPORT_RULES, MODEL };
