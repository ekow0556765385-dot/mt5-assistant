...
function attachLive(rows, journal) {
  const byT = {}; (journal || []).forEach(t => { byT[String(t.ticket)] = t; });
  (rows || []).forEach(r => {
    if (r.kind !== 'live') return;
    r.worth = liveWorth(r, byT[String(r.ticket)]);
    const b = liveBox(r);
    if (b) r.box = b;
  });
  return rows;
...
function calibrationSummary(calls, journal, stage) {
  ...
  function evidence(action) {
    ...
    (calls || []).filter(c => c.kind === 'live' && c.action === action && c.ticket && c.price_at).forEach(c => {
      const w = liveWorth(c, byTicket[String(c.ticket)]);            // the SAME arithmetic as each call's grade
...
module.exports = function mountArbiter(app, deps) {
  ...
  app.get('/api/arbiter/calls', gate, async (req, res) => {
    try {
      ...
      if (rows.some(r => r.kind === 'live') && getJournal)
        attachLive(rows, await getJournal(req.user.id).catch(() => []));
      const q = { good: 0, expensive: 0, ours: 0, luck: 0 };
  ...
  async function factsFor(uid, kind, period) {
    ...
    return buildFacts(kind, period, attachLive(cls.data || [], jr || []), eps.data || [], pos.data || [], jr || [], first);
  }
  ...
  async function history(uid, weeks) {
    ...
    const [cls, eps, acc, jr, pos] = await Promise.all([
      ...
      http.get(`${T.accounts}?user_id=eq.${uid}&select=*&order=first_seen_at.asc`, H()),
      getJournal ? getJournal(uid).catch(() => []) : Promise.resolve([]),
      http.get(`${T.positions}?user_id=eq.${uid}&select=account_number,symbol,raw_symbol,volume,risk_pct&limit=5000`, H())
    ]);
...
module.exports.progress = { liveWorth, liveBox, attachLive, progressSummary, accountsDetail, calibrationSummary, weeklyRows, rOf, median, PROG, METRICS };
