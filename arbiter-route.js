...
15b. PIPS in trades.list and in habit links are computed by Arbiter from each symbol's own pip size, so gold and EUR/USD are comparable. Each trade also carries its result in R (the "r" field) where it had a stop; prefer R when comparing trades.
  ...
  return {
    ...
    trades: {
      ...
      list: trades.slice(0, 30).map(t => ({ symbol: t.symbol, direction: t.direction, pl: round2(Number(t.total_pl) || 0),
        pips: pipsOf(t), r: rOf(t), opened: t.open_time, closed: t.close_time, session: t.session || null }))
    },
    // ── WEEKLY ONLY ────────────────────────────────────────────────
...
/* A trade's move in PIPS, computed here. The journal divides by the raw
   point and only by 10 on 5- or 3-digit symbols, so a 2-digit symbol like
   gold reported a $6.95 move as "694.50 pips" — which then sat in the same
   list as a 37-pip forex trade and made the written review nonsense.
   Arbiter knows each symbol's pip size, so it works it out itself. */
function pipsOf(t) {
  const open = Number(t.open_price), close = Number(t.close_price);
  if (!(open > 0) || !(close > 0)) return null;
  const sell = String(t.direction || '').toLowerCase().indexOf('sell') >= 0;
  return Math.round(((sell ? open - close : close - open) / pipOf(t.symbol)) * 10) / 10;
}
...
function enrichEpisodes(episodes, calls, journal, positions) {
  ...
  (episodes || []).forEach(e => {
    ...
    if (pos) {
      ...
      if (t) { link.resultPL = Number(t.total_pl) || 0; link.resultPips = pipsOf(t);
...
module.exports.progress = { tradeLookup, pipsOf, liveWorth, liveBox, attachLive, progressSummary, accountsDetail, calibrationSummary, weeklyRows, rOf, median, PROG, METRICS };
