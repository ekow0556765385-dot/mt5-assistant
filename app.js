...
app.post('/api/analyse', async (req, res) => {
  ...
  // ── SOURCE 2: SMC — fetch from smc-route ──────────────────────
  let smcData = {};
  try {
    const r = await axios.get(`http://localhost:${PORT}/smc/${sym}`, { timeout: 3000 });
    smcData = r.data || {};
  } catch(e) {
    try {
      const r2  = await axios.get(`http://localhost:${PORT}/smc`, { timeout: 3000 });
      const all = r2.data || {};
      smcData   = all[sym] || Object.values(all)[0] || {};
    } catch(e2) { smcData = {}; }
  }
...
1. Price + Indicators — live price, RSI, EMA values, bias_score from the MT5 EA
2. SMC — Smart Money Concepts: CHoCH, BOS, Order Blocks, FVGs, HTF bias
3. Pattern Detector — candlestick pattern detections with confidence %
