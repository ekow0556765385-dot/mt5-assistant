...
app.post('/api/multi-candles', async (req, res) => {
  ...
  const cs = getCandlesStore(userId);
  const summary = {};
  if (!s.indicatorsBySymbol) s.indicatorsBySymbol = {}; // per-symbol, distinct from the flat s.indicators used for the EA's own attached pair elsewhere

  Object.entries(d.symbols).forEach(([sym, payload]) => {
    if (!payload || !Array.isArray(payload.candles) || !payload.timeframe) return;
    ...
    if (payload.indicators) s.indicatorsBySymbol[sym] = payload.indicators;

    cs[sym] = {
      symbol: sym, timeframe: tf, candles: payload.candles,
      timestamp: d.timestamp || new Date().toISOString(),
      received_at: new Date().toISOString()
    };

    broadcast(userId, 'CANDLE_UPDATE', { symbol: sym, timeframe: tf, candles: payload.candles, patterns: s.patterns[sym][tf], indicators: payload.indicators || null });
    summary[sym] = payload.candles.length;
  });
