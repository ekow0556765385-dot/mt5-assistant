...
function normaliseClosedTrade(t, idx) {
  ...
  // ── Enrich missing prices from all available sources ────────────
  // Source A: open trade snapshot (captured when trade was live)
  // Source B: estimate close_price from profit + lots + pip value
  // Source C: estimate open_price from close_price + pips (reverse)
  ...
  // Source A: snapshot lookup by ticket
  ...
  let pipSize = 0.0001; // default forex
  ...
  // Source B: estimate close_price from open_price + pips
  ...
  // Source C: estimate open_price from profit + lots + pip value
  // profit = lots × pips × pipValue  →  pips = profit / (lots × pipValue)
  // Then open = close ∓ pips×pipSize  (but we don't have close either in this case)
  // So use: pipValue ≈ lots × 10 for standard account (USD base)
  // open_price ≈ derived from known profit and assumed pip value
  // This is the last resort — marks as estimated
  if (!enriched_open && profit !== 0 && effectiveLots > 0) {
    // We know: profit = pips × pipValue, pipValue = lots × (1/pipSize) × 0.00001 (approx)
    // For simplicity use pip value = lots × 10 for majors (1 pip = $10 per standard lot)
    const approxPipValue = effectiveLots * 10;
    const estimatedPips  = profit / approxPipValue;
    // We can't get open_price without a reference — skip this, just note it
    // close_price also can't be estimated without open_price
    // Leave null — MathReporter is required for these old trades
  }
...
app.get('/api/math-data', (req, res) => {
  try {
    ...
    if (mathClosed.length > 0) {
      // MathReporter data exists — merge with bridge + re-enrich null prices
      const mathByTicket = {};
      mathClosed.forEach(t => {
        // Re-run normaliseClosedTrade on any stored trade missing prices
        // This picks up snapshot data captured since the trade was stored
