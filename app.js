...
// ── Pattern filter helper — shared by /api/analyse ────────────────
// Mirrors the same logic as agent-module.js buildFilteredPatternData()
// so the Brain and the autonomous agent use identical filtering rules.
function buildFilteredPatternData(patternData) {
  ...
  // Filter candles to trend-aligned only — sort newest first
  let sorted = [...candles];
...
// ── /api/analyse — MT5 Trading Brain verdict ──────────────────────
// PATCHED: pattern data is now pre-filtered before reaching Haiku.
// System prompt updated with SOURCE 5 rules matching agent-module.js.
// Everything else (SMC payload, SSI, candles, verdict extraction) unchanged.
app.post('/api/analyse', async (req, res) => {
  ...
  // ── Filter pattern data before sending to Haiku ───────────────
  // Splits by age, filters candles to bias-aligned only, computes flags.
  // Haiku only sees recent_patterns (bar_index 0-2) and aligned candles.
...
1. RECENT PATTERNS ONLY: Only treat patterns in "recent_patterns" (bar_index 0–2) as valid signals.
   - "mid_patterns_count" = patterns 3–8 bars old. Background context only — do not trade on these alone.
   - "stale_patterns_count" = patterns 9+ bars old. These are EXPIRED — ignore completely.

2. CANDLE ALIGNMENT: "aligned_candles" contains only candles matching the current bias direction.
   - If "has_candle_alignment" is false, candle structure does NOT confirm the bias. Treat as weak.
   - Empty aligned_candles = recent price action is moving against the bias.

3. PATTERN-BIAS CONFLICT: If "pattern_bias_conflict" is true, a recent pattern is pointing AGAINST
   the bias direction. This is a direct contradiction — state this clearly and lean toward NEUTRAL.

4. HARD GATE: If "has_recent_confirmation" is false (no patterns in last 2 bars), state that
   pattern confirmation is absent. This should lower your confidence to Low regardless of other sources.

5. S/R only: Use sr_levels for key levels. There are no demand/supply zones — do not reference them.
...
📈 CONFLUENCE ANALYSIS
[How all 5 sources agree or conflict — explicitly state whether SOURCE 5 has fresh pattern confirmation]
