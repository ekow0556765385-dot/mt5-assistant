...
const DOWNLOAD_CATALOG = [
  ...
  { id: 'bridge-watcher',   name: 'File Bridge Watcher',               path: 'mt5_bridge_watcher.exe',                 audience: 'exness' },
...
app.get('/api/downloads/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const folders = [];
      if (askedFolder) {
        folders.push(askedFolder);
        rootEntries.forEach(n => {
          if (n.toLowerCase() === askedFolder.toLowerCase() && n !== askedFolder) folders.push(n);
        });
      }
      folders.push('');
      ...
      for (const folder of folders) {
        ...
        const pick =
          ...
          names.filter(n => n.toLowerCase().endsWith(ext.toLowerCase()) &&
                            n.toLowerCase().startsWith(stem))
               .sort().reverse()[0];
