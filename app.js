...
app.get('/api/downloads/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const objectUrl = `${SUPABASE_URL}/storage/v1/object/downloads/${entry.path}`;
    const filename  = entry.path.split('/').pop();

    let upstream;
    try {
      upstream = await axios.get(objectUrl, {
        headers: supabaseServiceHeaders(),
        responseType: 'stream',
        maxRedirects: 5,
      ...
      console.error(`[DOWNLOADS] upstream ${code || 'ERR'} for ${entry.path}`);
      if (code === 404) {
