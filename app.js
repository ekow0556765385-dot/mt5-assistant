  ...
  try {
    ...
    try {
      ...
      if (e.response?.status !== 404) {
        console.error(`[DOWNLOADS] upstream ${e.response?.status || 'ERR'} for ${entry.path}`);
        return res.status(502).json({ error: 'Could not read the file from storage' });
      }

      // Exact miss — try to resolve by stem within the same folder.
      const folder  = entry.path.includes('/') ? entry.path.slice(0, entry.path.lastIndexOf('/')) : '';
      const wanted  = entry.path.split('/').pop();
      const ext     = wanted.slice(wanted.lastIndexOf('.'));
      // Stem = name up to the first version-ish separator, lowercased.
      ...
      try {
        const { data: listing } = await axios.post(
          `${SUPABASE_URL}/storage/v1/object/list/downloads`,
          { prefix: folder, limit: 200, sortBy: { column: 'name', order: 'desc' } },
          { headers: supabaseServiceHeaders({ 'Content-Type': 'application/json' }) }
        );
        const candidates = (listing || [])
          ...
          .sort()                      // newest version last…
          .reverse();                  // …so take the highest
        if (candidates.length) match = (folder ? folder + '/' : '') + candidates[0];
      } catch (le) {
        console.error('[DOWNLOADS] bucket list failed:', le.response?.status || le.message);
      }

      if (!match) {
        console.error(`[DOWNLOADS] 404 and no stem match for ${entry.path} (stem="${stem}")`);
        return res.status(404).json({
          error: `File not uploaded yet: ${entry.path}. Upload it to the "downloads" bucket.`
        });
      }
...
function resolveSource(req) {
  ...
  const newest = listSources(req.user.id)[0];
  return newest ? newest.sourceId : null;
}
...
app.get('/api/sources', requirePlan('pro'), (req, res) => {
  res.json({ sources: listSources(req.user.id), active: resolveSource(req) });
});
