...
app.post('/api/update', (req, res) => {
  ...
  if (d.accountInfo && d.accountInfo.balance) {
    try {
      ...
      if (d.openTrades) mf.open_trades = d.openTrades.map(normaliseOpenTrade);
...
function normaliseAccount(a) {
  ...
  return {
    ...
    login:       a.login     || a.account_login     || 0,
...
function normaliseClosedTrade(t, idx) {
  if (!t) return null;
  const open_price  = parseFloat(t.open_price  || t.openPrice  || t.entry_price || 0) || null;
  const close_price = parseFloat(t.close_price || t.closePrice || t.exit_price  || 0) || null;
  ...
  const close_time  = t.close_time  || t.closeTime  || t.exit_time  || '';
...
app.get('/api/math-data', (req, res) => {
  try {
    ...
    // ── CLOSED TRADES: MathReporter is the gold standard ──
    // Priority 1: math_trades.json (populated by MathReporter via either channel)
    // Priority 2: s.closedTrades from bridge (has open+close price in camelCase)
    // Priority 3: agent_session executed_orders (has entry price only, no close price)
    let closedTrades = math.closed_trades || [];
    let stats        = math.stats         || {};
    let dataSource   = 'math_reporter';

    if (closedTrades.length === 0 && s.closedTrades && s.closedTrades.length > 0) {
      // Bridge sends closedTrades with openPrice + closePrice — normalise them
      closedTrades = s.closedTrades.map((t, i) => normaliseClosedTrade({
        ...t,
        open_price:  t.openPrice  || t.open_price,
        close_price: t.closePrice || t.close_price,
        _source:     'bridge'
      }, i)).filter(t => t !== null);
