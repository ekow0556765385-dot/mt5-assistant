...
// ── Telegram config ───────────────────────────────────────────────
const TELEGRAM_TOKEN    = '8591020831:AAF7m22h7gwmuDWklvbRvnXtpPlNolScwWw';
...
// ── NEW v3.4 — candles store for /api/candles (AI Trading Brain) ──
let candlesStore = {};   // stores latest M15 candle batch per symbol
...
// ── /api/candles — handles BOTH formats: ─────────────────────────
//   Format A (EA v3.4): POST { symbol, timeframe, candles: [...] }
//   Format B (old):     POST { symbol, candles: { H1: [...], M15: [...] } }
//   GET  — returns latest candle batch (used by AI Trading Brain)
// ─────────────────────────────────────────────────────────────────
app.post('/api/candles', (req, res) => {
  ...
  // Format A — EA v3.4: flat candle array with timeframe field
  if (Array.isArray(d.candles) && d.timeframe) {
    const tf = d.timeframe;
    s.candles[sym][tf]  = d.candles;
    s.patterns[sym][tf] = runPatternDetection(sym, tf, d.candles);

    // Store in candlesStore for GET /api/candles (AI Trading Brain)
    candlesStore[sym] = {
      symbol:      sym,
      timeframe:   tf,
      candles:     d.candles,
  ...
  // Format B — old multi-timeframe object: { H1: [...], M15: [...] }
  ...
  return res.status(400).json({ error: 'candles must be an array (v3.4) or object (multi-tf)' });
});

// GET /api/candles — returns latest candle batch for AI Trading Brain
// Optional query: ?symbol=EURUSD  →  returns that symbol's latest batch
// No query       →  returns all stored symbols
app.get('/api/candles', (req, res) => {
  const { symbol } = req.query;
  if (symbol) {
    const data = candlesStore[symbol];
    if (!data) return res.json({ symbol, candles: [], note: 'No candle data received yet for this symbol' });
    return res.json(data);
  }
  // Return the most recently updated symbol if no query param
  const all = Object.values(candlesStore);
  if (!all.length) return res.json({ candles: [], note: 'No candle data received yet' });
