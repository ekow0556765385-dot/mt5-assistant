...
app.get('/api/downloads/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Supabase Storage answers 400 (not 404) for an object that is not
      // there. The fallback below was gated on 404 only, so for every missing
      // file it bailed straight to 502 and the resolver never ran - which is
      // why 8 of 10 downloads failed while the resolver looked "fine".
      const st = e.response?.status;
      if (st !== 404 && st !== 400) {
        console.error(`[DOWNLOADS] upstream ${st || 'ERR'} for ${entry.path}`);
        return res.status(502).json({ error: 'Could not read the file from storage' });
      }
      console.warn(`[DOWNLOADS] ${st} for ${entry.path} — resolving against the bucket…`);
...
app.put('/api/watch-pairs', requireAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      return res.status(503).json({ error: 'Pair saving is not enabled yet.', pairs: clean, limit, plan });
    }
