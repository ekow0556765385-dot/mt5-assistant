...
app.post('/api/analyse', async (req, res) => {
  ...
  let smcContextBlock = '';
  if (smcContext && (smcContext.orderBlocks || smcContext.sweepSummary)) {
    const obs  = smcContext.orderBlocks || [];
    const fvgs = smcContext.fvgs        || [];
    const str  = smcContext.structure   || [];

    const obLines = obs.map(ob =>
      `  ${ob.direction} OB: ${ob.low} – ${ob.high}${ob.mitigated ? ' [mitigated]' : ' [active]'}${ob.timeStart ? ' @ ' + ob.timeStart : ''}`
    ).join('\n') || '  None';

    const fvgLines = fvgs.map(f =>
      `  ${f.direction} FVG: ${f.low} – ${f.high}${f.filled ? ' [filled]' : ' [unfilled]'}${f.timeStart ? ' @ ' + f.timeStart : ''}`
    ).join('\n') || '  None';

    const strLines = str.map(sv =>
      `  ${sv.type} @ ${sv.level} (${sv.time || ''})`
    ).join('\n') || '  None';

    smcContextBlock = [
      `\n=== SOURCE 5: SMC Liquidity Sweep Status ===`,
      `Current price  : ${smcContext.currentPrice || 'unknown'}`,
      `Timeframe      : ${smcContext.timeframe    || 'unknown'}`,
      `Sweep status   : ${smcContext.sweepStatus  || 'unknown'}`,
      `Sweep confirmed: ${smcContext.sweepConfirmed === true ? 'YES' : smcContext.sweepConfirmed === false ? 'NO' : 'unknown'}`,
      ``,
      `SWEEP ASSESSMENT:`,
      `${smcContext.sweepSummary || 'No sweep data available.'}`,
      ``,
      `Active Order Blocks:`,
      obLines,
      ``,
...
CRITICAL SWEEP RULE: If SOURCE 6 is present and sweepConfirmed is NO or sweepStatus is "warning":
- Do NOT recommend immediate entry
- Warn explicitly that a liquidity hunt may occur before the real move
- State clearly that the OB has not been swept yet
- Advise waiting for the sweep (wick beyond the OB level) and rejection before entering
- Use NEUTRAL as the verdict and note it is a WAIT situation

If sweepConfirmed is YES or sweepStatus is "swept", treat this as additional confluence confirming entry.
If SOURCE 6 is absent, proceed with normal analysis from sources 1-5.
