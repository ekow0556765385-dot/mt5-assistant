...
// ── Auth middleware ────────────────────────────────────────────
const { requirePlan, requireAuth, getMe, validateKey, regenerateKey, issueTicketRoute, isOwner, getUserCredits, deductCredits, resetUserCredits, MONTHLY_CREDIT_USD, SUPABASE_URL, supabaseServiceHeaders } = require('./auth-middleware');
...
const { getUserIdForLicenceKey, insertTrade, getTradesForUser } = require('./journal-store');
...
app.post('/journal/trade', async (req, res) => {
  ...
  try {
    ...
    broadcast('journal_update', { ticket: trade.ticket, symbol: trade.symbol, direction: trade.direction, totalPL: trade.totalPL, closeTime: trade.closeTime });
...
// Used by the Pro dashboard's Trading Journal table
app.get('/api/journal', requirePlan('pro'), async (req, res) => {
  try {
    const entries = await getTradesForUser(req.user.id);
    res.json({ entries });
  } catch (err) {
    console.error('[JOURNAL ERROR]', err.message);
    res.status(500).json({ error: err.message });
