...
// ── NEW: Pattern Detector dashboard ──────────────────────────────
...
let s = {
  ...
  patternAlerts: [],
  // NEW: live pattern detector state (fed by PatternDetector.mq5 indicator)
  livePatterns: {}
};
...
// ── NEW: Pattern Detector live feed (from PatternDetector.mq5) ───
//
//  POST /api/patterns  — MT5 indicator posts here on every new bar
//  GET  /api/patterns  — dashboard polls here every 5 seconds
//  GET  /api/patterns/latest — returns most recently received symbol
//
app.post('/api/patterns', (req, res) => {
  ...
  // Also broadcast to any open WebSocket clients (index.html can listen too)
...
app.get('/api/patterns', (req, res) => {
  ...
  if (symbol && tf) {
    ...
    return res.json(s.livePatterns[key] || {});
  }
...
async function fetchNews() {
  ...
  try {
    ...
    if (events.length > 0) { s.newsEvents = events; broadcast('NEWS_UPDATE', events); console.log(`✓ Fetched ${events.length} HIGH impact events this week`); return; }
  ...
  try {
    ...
    const events = mapEvents(data);
    s.newsEvents = events; broadcast('NEWS_UPDATE', events); console.log(`✓ Fetched ${events.length} HIGH impact events next week`);
  } catch (e) {
...
app.post('/journal/trade', async (req, res) => {
  ...
  try {
    ...
    broadcast('journal_update', {
      ...
      direction: trade.direction, totalPL: trade.totalPL,
      closeTime: trade.closeTime, row: result.row
    });
...
http.listen(3000, '0.0.0.0', () => {
  ...
  ║   Telegram: commands + 70% conf filter       ║
