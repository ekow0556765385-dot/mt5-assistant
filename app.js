...
// API key lives server-side as ANTHROPIC_API_KEY env variable on Railway
app.post('/api/analyse', async (req, res) => {
  const { update, smc, patterns, candles, ssi } = req.body;
  if (!update && !smc && !patterns) {
    return res.status(400).json({ error: 'No data provided' });
  }

  const prompt = [
    'Live MT5 market data from all 4 sources:\n',
    '=== SOURCE 1: /api/update (price + indicators) ===',
    JSON.stringify(update   || {}, null, 2),
    '\n=== SOURCE 2: /smc (Smart Money Concepts) ===',
    JSON.stringify(smc      || {}, null, 2),
    '\n=== SOURCE 3: /api/patterns (EA detections) ===',
    JSON.stringify(patterns || {}, null, 2),
    '\n=== SOURCE 4: SSI Engine (Structure Signal Indicator) ===',
    JSON.stringify(ssi      || {}, null, 2),
    '\nAnalyse all 4 sources and give your verdict.'
  ].join('\n');
  ...
  try {
    const response = await axios.post(
      'https://api.anthropic.com/v1/messages',
      {
        ...
        max_tokens: 1000,
        system: `You are an expert Forex and Gold market analyst with deep knowledge of Smart Money Concepts (SMC), EMA trend analysis, candlestick patterns, and structure trading.

You receive data from 4 sources:
1. /api/update — live price, RSI, EMA values, bias_score from the MT5 EA
2. /smc — Smart Money Concepts: CHoCH, BOS, Order Blocks, FVGs, HTF bias
3. /api/patterns — candlestick pattern detections with confidence %
4. SSI Engine — Structure Signal Indicator: signal codes +1/-1 (Structure+Trend), +2/-2 (Pattern+Trend), 0 (no signal)
...
📈 CONFLUENCE ANALYSIS
[How all 4 sources agree or conflict. Name SSI signal, SMC bias, RSI, pattern names + confidence %]

🎯 VERDICT — [BULLISH / BEARISH / NEUTRAL] | Confidence: [Low/Medium/High]
[Trade setup: direction, entry logic, SL concept, invalidation]

⚠️ KEY RISK
[1-2 sentences on main risk or conflicting signal]`,
