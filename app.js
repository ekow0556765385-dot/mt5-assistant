...
app.post('/api/update', async (req, res) => {
  ...
  if (d.openTrades) {
    ...
    d.openTrades.forEach(t => {
      ...
      const snapshots = getOpenSnapshots(userId);
    ...
    if (snapshotChanged) saveOpenSnapshots(userId);
  }
  ...
  if (d.accountInfo && d.accountInfo.balance) {
    try {
      const mf          = loadMathTrades(userId);
      ...
      if (loginChanged || currencyChanged || balanceJumped) {
        ...
        console.log(`[MATH] user=${userId} account switched (${reason}) — clearing this user's state`);

        mf.closed_trades = [];
        mf.open_trades   = [];
        mf.stats         = {};

        setOpenSnapshots(userId, {});
        saveOpenSnapshots(userId);
        ...
        s.indicators   = {};
        s.patternAlerts= [];
        broadcast(userId, 'ACCOUNT_SWITCH', { reason, old_login: storedLogin, new_login: newLogin });
        ...
        console.log(`[MAIN] user=${userId} broadcast account switch + cleared state to that user's dashboards only`);
      }
...
// Per-user storage: math_trades.json / open_trade_snapshots.json used to be
// one shared file for the entire server — meaning every logged-in customer
// was reading and writing the SAME trade history regardless of whose EA
// sent it. Now each user gets their own file under user_data/.
const USER_DATA_DIR = path.join(__dirname, 'user_data');
try { if (!fs.existsSync(USER_DATA_DIR)) fs.mkdirSync(USER_DATA_DIR, { recursive: true }); } catch(e) {}

function mathTradesFilePath(userId) {
  return path.join(USER_DATA_DIR, `math_trades_${userId}.json`);
}
function openSnapshotFilePath(userId) {
  return path.join(USER_DATA_DIR, `open_trade_snapshots_${userId}.json`);
}

const snapshotsByUser = {};
function getOpenSnapshots(userId) {
  if (!snapshotsByUser[userId]) {
    let loaded = {};
    try {
      const f = openSnapshotFilePath(userId);
      if (fs.existsSync(f)) {
        loaded = JSON.parse(fs.readFileSync(f, 'utf8'));
        console.log(`[MATH] user=${userId} loaded ${Object.keys(loaded).length} open trade snapshots`);
      }
    } catch(e) {}
    snapshotsByUser[userId] = loaded;
  }
  return snapshotsByUser[userId];
}
function setOpenSnapshots(userId, obj) { snapshotsByUser[userId] = obj; }
function saveOpenSnapshots(userId) {
  try { fs.writeFileSync(openSnapshotFilePath(userId), JSON.stringify(getOpenSnapshots(userId), null, 2)); } catch(e) {}
}

function loadMathTrades(userId) {
  try {
    const f = mathTradesFilePath(userId);
    if (fs.existsSync(f)) return JSON.parse(fs.readFileSync(f, 'utf8'));
  } catch (e) { console.error(`[MATH] user=${userId} load error:`, e.message); }
  return { account: {}, closed_trades: [], open_trades: [], stats: {}, last_update: null };
}

function saveMathTrades(userId, data) {
  const target = mathTradesFilePath(userId);
...
function normaliseClosedTrade(userId, t, idx) {
  if (!t) return null;
  ...
  const snap = getOpenSnapshots(userId)[String(ticket)];
  if (snap) {
...
app.post('/api/math-trades', async (req, res) => {
  try {
    ...
    if (!userId) {
      console.warn('[MATH] Rejected — no valid licenceKey. Update MathReporter.mq5 to v1.3+ and paste a valid licence key.');
...
app.get('/api/math-data', requirePlan('pro'), (req, res) => {
  try {
    ...
    const math   = loadMathTrades(userId);
    const st     = getState(userId);
    ...
    let bridgeClosed = (st.closedTrades || []).map((t, i) => normaliseClosedTrade(userId, {
      ...t,
    ...
    if (mathClosed.length > 0) {
      ...
      mathClosed.forEach(t => {
        if (!t.open_price || t.open_price === 0) {
          const re = normaliseClosedTrade(userId, { ...t }, 0);
    ...
    if (closedTrades.length === 0 && agentDataAppliesToThisUser) {
      try {
        ...
        if (fs.existsSync(sessionFile)) {
          ...
          if (orders.length > 0) {
            closedTrades = orders.map((o, i) => normaliseClosedTrade(userId, { ...o, type: o.action || o.type, _source: 'agent_session' }, i)).filter(t => t !== null);
            stats      = recalcMathStats(closedTrades);
...
app.delete('/api/math-trades/reset', requirePlan('pro'), (req, res) => {
  try {
    saveMathTrades(req.user.id, { account: {}, closed_trades: [], open_trades: [], stats: {}, last_update: new Date().toISOString() });
    setOpenSnapshots(req.user.id, {});
    saveOpenSnapshots(req.user.id);
