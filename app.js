...
// ── Auth middleware ────────────────────────────────────────────
const { requirePlan, requireAuth, getMe, validateKey, regenerateKey, setBrokerType, cancelSubscription, issueTicketRoute, isOwner, getUserCredits, deductCredits, resetUserCredits, MONTHLY_CREDIT_USD, SUPABASE_URL, supabaseServiceHeaders, verifyAppSession, getUserIdForLicenceKey } = require('./auth-middleware');
...
app.get('/api/quotes', (req, res) => {
  const STALE_MS = 5 * 60 * 1000; // drop anything older than 5 minutes
  const now = Date.now();
...
// ── /account — clean URL for the account page ─────────────────────
// The page used to be reached as ".../account.html" on the static host,
// which (a) looks unprofessional in the address bar and (b) put it on a
// DIFFERENT ORIGIN from this API — so the session cookie was not sent
// with /api/downloads calls and they came back 401. Serving it from here
// makes it same-origin, so downloads authenticate like every other route.
// Deliberately NOT behind requirePlan: free users must be able to reach
// their own account page to upgrade.
app.get('/account', (req, res) => res.sendFile(path.join(__dirname, 'account.html')));
// Keep the old path working for anyone with it bookmarked or linked.
app.get('/account.html', (req, res) => res.redirect(301, '/account'));
...
const DOWNLOAD_CATALOG = [
  ...
  { id: 'bridge-watcher',   name: 'File Bridge Watcher',               path: 'bridge/mt5_bridge_watcher.exe',                 audience: 'exness' },
  { id: 'setup-guide',      name: 'Setup Guide',                       path: 'docs/setup-guide.pdf',                          audience: 'all'    },
  { id: 'dashboard-guide',  name: 'Dashboard Guide',                   path: 'docs/dashboard-guide.pdf',                      audience: 'all'    },
];
...
app.get('/api/downloads/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Mint signed URL (5 minutes — long enough to download, short enough to be useless if shared)
    ...
    // signed.signedURL is already a full path starting with /storage/v1/object/sign/...
    // Just prefix the Supabase project URL — do NOT add /storage/v1 again.
    const signedPath = signed.signedURL || signed.signed_url || signed.url;
    if (!signedPath) {
      console.error('[DOWNLOADS] No signed URL in response:', JSON.stringify(signed));
      return res.status(500).json({ error: 'Could not generate download link' });
    }

    // If Supabase returns a full https URL, use it as-is; otherwise prefix our base URL
    let downloadUrl = signedPath.startsWith('http')
      ? signedPath
      : `${SUPABASE_URL}${signedPath}`;

    // Force an ATTACHMENT download. Without ?download=, Supabase serves the
    // object inline with its own content-type — the browser NAVIGATES to it
    // instead of downloading, which is why a click sometimes flashed a
    // different page (and why .pdf guides opened in the viewer rather than
    // saving). The value becomes the suggested filename.
    const suggestedName = entry.path.split('/').pop();
    downloadUrl += (downloadUrl.includes('?') ? '&' : '?') +
                   'download=' + encodeURIComponent(suggestedName);

    console.log(`[DOWNLOADS] url issued: user=${req.user.id} file=${entry.id}`);
    res.json({ url: downloadUrl, name: entry.name, filename: suggestedName });
  } catch (e) {
...
async function fetchExternalQuotes() {
  if (!TWELVEDATA_API_KEY) {
    console.warn('[QUOTES] TWELVEDATA_API_KEY not set — ticker will only show prices from a connected EA.');
    return;
  }
  try {
    const { data } = await axios.get('https://api.twelvedata.com/quote', {
      params: {
        symbol: EXTERNAL_QUOTE_SYMBOLS.join(','),
        apikey: TWELVEDATA_API_KEY,
      },
      timeout: 8000,
    });

    // TwelveData returns a single object (not keyed by symbol) when only
    // one symbol resolves, and an object keyed by symbol for multiple —
    // normalize both shapes into one array to iterate.
    const rows = data && data.symbol ? [data] : Object.values(data || {});

    rows.forEach(row => {
      if (!row || row.status === 'error' || !row.symbol) return;
      const ourSymbol = row.symbol.replace('/', ''); // 'EUR/USD' -> 'EURUSD'
      const price = parseFloat(row.close ?? row.price);
      if (isFinite(price)) updateQuoteCache(ourSymbol, price);
    });
  } catch (e) {
    console.warn('[QUOTES] External price fetch failed:', e.response?.data || e.message);
  }
}

// Run once at startup, then every 30s — TwelveData's free tier allows
// 8 requests/minute, this uses 2.
fetchExternalQuotes();
setInterval(fetchExternalQuotes, 30000);
...
app.post('/api/multi-candles', async (req, res) => {
  ...
  Object.entries(d.symbols).forEach(([sym, tfBundle]) => {
    ...
    Object.entries(tfBundle).forEach(([tf, payload]) => {
      ...
      s.patterns[sym][tf] = runPatternDetection(userId, sym, tf, payload.candles);
    ...
    broadcast(userId, 'CANDLE_UPDATE', { symbol: sym, candles: candlesByTF, patterns: patternsByTF, indicators: indicatorsByTF });
