...
These two lines must be the very last lines of your response.`,
    ...
    // ── Extract verdict + confidence server-side ──────────────────
    // Catches every format Claude might use — done once here so
    // the browser never has to guess
    const clean = text.replace(/\*\*/g, '').replace(/\n/g, '
');

    // VERDICT: try explicit line first, then section headers, then any mention
    let verdict = 'NEUTRAL';
    const vPatterns = [
      /^VERDICT:\s*(BULLISH|BEARISH|NEUTRAL)\s*$/im,           // clean line
      /VERDICT\s*[—:\-]+\s*(BULLISH|BEARISH|NEUTRAL)/i,        // VERDICT — BEARISH
      /##[^#
]*?(BULLISH|BEARISH|NEUTRAL)[^#
]*(?:verdict|setup|bias)/i, // ## header
      /(?:overall|final|my)\s+(?:verdict|bias|view)\s*[:\-—]*\s*(BULLISH|BEARISH|NEUTRAL)/i,
      /direction[:\s]*(BULLISH|BEARISH|NEUTRAL)/i,
      /verdict[^.!?
]{0,60}(BULLISH|BEARISH|NEUTRAL)/i,
    ];
    for (const p of vPatterns) {
      const m = clean.match(p);
      if (m) { verdict = m[1].toUpperCase(); break; }
    }
    // Last resort — count mentions and pick the dominant one
    if (verdict === 'NEUTRAL') {
      const bullCount = (clean.match(/BULLISH/gi) || []).length;
      const bearCount = (clean.match(/BEARISH/gi) || []).length;
      if (bearCount > bullCount + 1) verdict = 'BEARISH';
      else if (bullCount > bearCount + 1) verdict = 'BULLISH';
    }

    // CONFIDENCE: try explicit line first, then any mention
    let confidence = 'Medium';
    const cPatterns = [
      /^CONFIDENCE:\s*(Low|Medium|High)\s*$/im,                 // clean line
      /Confidence[:\s]+\*{0,2}(Low|Medium|High)\*{0,2}/i,      // Confidence: Medium
      /(Low|Medium|High)\s*(?:confidence|conviction|probability)/i,
      /confidence[^.!?
]{0,40}(Low|Medium|High)/i,
    ];
    for (const p of cPatterns) {
      const m = clean.match(p);
      if (m) { confidence = m[1].charAt(0).toUpperCase() + m[1].slice(1).toLowerCase(); break; }
    }
