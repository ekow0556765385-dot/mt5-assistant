...
const TELEGRAM_MIN_CONF = 70;   // Only send alerts >= this confidence %
...
async function pollTelegramCommands() {
  try {
    ...
    // Silent fail — polling errors don't crash the server
  }
}
...
function runPatternDetection(symbol, timeframe, candleArray) {
  ...
  // Deduplicate — only alert if pattern changed
  ...
  // ── Telegram alert (confidence filter applied) ────────────────
