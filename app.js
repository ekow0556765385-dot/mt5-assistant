...
const liveOverlap = require('./live-overlap');
...
// Messages (account page). Behind requireAuth, NOT requirePlan: a
// customer whose dashboard is paused is exactly the person who most needs
// to reach you, and a free customer should be able to ask a billing
// question. The admin side is mounted from admin-route.js.
const supportRoute = require('./support-route');
app.use(supportRoute.customerRouter(requireAuth));
...
app.get('/arbiter',    requireFramedByDashboard, requirePlan('pro'), (req, res) => res.sendFile(path.join(__dirname, 'arbiter.html')));

// Arbiter's two scripts. Served the same way as bw-source.js: no plan gate,
// because they are static code with no data in them, and gating them would
// break the page for a user whose session is mid-refresh. The DATA they
// fetch is gated as usual.
app.get('/arbiter-engine.js', (req, res) => res.sendFile(path.join(__dirname, 'arbiter-engine.js')));
app.get('/arbiter-feeds.js',  (req, res) => res.sendFile(path.join(__dirname, 'arbiter-feeds.js')));
// The retracement engine, so Arbiter can run the SAME pullback-vs-reversal
// assessment the Pattern Detector shows, instead of reporting "no reading".
app.get('/retracement.js', (req, res) => res.sendFile(path.join(__dirname, 'retracement.js')));
...
// clean +1h reading.
// PER USER, not global. These were two single variables shared by every
// customer: with several EAs posting, whichever beat arrived first each
// minute did the sampling and everybody else was skipped until they
// happened to be first. On a busy account a verdict could sit unsampled
// for hours. Keyed by user id, each trader gets their own once-a-minute.
const brainResolveBusy = new Set();
const brainResolveLast = new Map();
async function resolveDueVerdicts(userId, sourceId) {
  if (!userId) return;
  if (brainResolveBusy.has(userId)) return;
  if (Date.now() - (brainResolveLast.get(userId) || 0) < 60 * 1000) return;   // at most once a minute, per user
  brainResolveBusy.add(userId);
  brainResolveLast.set(userId, Date.now());
  try {
    ...
    brainResolveBusy.delete(userId);
  }
}

// ── ARBITER (Phase 1) ─────────────────────────────────────────────
// Trade Errors as timed episodes, MFE/MAE per position, and the account
// timeline — recorded off the EA heartbeat below. See arbiter-route.js.
// getNews and getRiskSettings are FUNCTIONS on purpose: newsEvents is a
// `let` declared further down this file and settings-store is required
// further down too, so reading either here would throw at startup. They
// are only called on heartbeats, long after the file has finished loading.
const arbiter = require('./arbiter-route')(app, {
  requirePlan, getState, getCandlesStore, livePriceFor, normalisePair, resolveSource,
  getNews: () => newsEvents,
  getRiskSettings: (userId) => require('./settings-store').getSettingsForUser(userId),
  // Reports bill exactly like /api/analyse: same credit check, same deduction.
  getUserCredits, deductCredits,
  getJournal: (userId) => require('./journal-store').getTradesForUser(userId, { limit: 1000 }),
  // Automatic reports: the SAME entitlement test requirePlan uses, and the
  // node-cron instance app.js already runs its own schedules on.
  cron,
  getUserPlan: (...a) => require('./auth-middleware').getUserPlan(...a),
  accessState: (...a) => require('./auth-middleware').accessState(...a),
  planRank:    (...a) => require('./auth-middleware').planRank(...a),
  sharingBlocked: (...a) => require('./auth-middleware').sharingBlocked(...a),
  SUPABASE_URL, supabaseServiceHeaders
});
...
function runPatternDetection(userId, sourceId, symbol, timeframe, candleArray) {
  ...
  // DEDUP BY CANDLE, NOT BY NAME. Up to four feeds (/api/update,
  // /api/candles single + bundle, /api/multi-candles) run this detector on
  // the same pair+timeframe with DIFFERENT history lengths (the EA sends 50
  // bars on one, 60 on another). The same closed candle can then read as a
  // Doji on one feed and an Inside Bar on the other, and remembering only
  // the NAME made every alternation look new — the same candle alerted over
  // and over to Telegram, the Assistant and Arbiter. Now: at most one alert
  // per pair+timeframe per candle. A pattern alerts only if it sits on a
  // NEWER candle than the last one alerted. If a feed sends no readable bar
  // time, the old name rule is used rather than failing.
  const bar  = candleArray[latest.index] || {};
  const rawT = bar.t !== undefined ? bar.t : bar.time;
  let barT = Number(rawT);
  if (!Number.isFinite(barT)) { const p = Date.parse(String(rawT || '').replace(/\./g, '-')); barT = Number.isFinite(p) ? p / 1000 : null; }
  const prev = lastAlerted[key];
  if (barT != null && prev && prev.barT != null) {
    if (barT <= prev.barT) return detected;
  } else if (prev && prev.name === latest.name) {
    return detected;
  }
  ...
  if (!primedAfterBoot.has(primeKey)) {
    ...
    lastAlerted[key] = { barT, name: latest.name };
...
app.post('/api/update', async (req, res) => {
  ...
  // Two accounts on one licence posting at the same moment means two
  // terminals are running at once. live-overlap records it; the nightly
  // sharing sweep decides whether it matters (different machines) or not
  // (one PC running several prop terminals).
  liveOverlap.note(userId, liveAccountNumber);
  ...
  // Arbiter rides the same heartbeat, also deliberately not awaited.
...
app.post('/api/analyse', requirePlan('pro'), async (req, res) => {
  const { ssi, smc: smcPayloadFromBrain, maxTokens, symbol, timeframe, confluence } = req.body;
  ...
  /* ── SOURCE 6 · THE COMBINED READ ────────────────────────────────
     Published by the Pattern Detector's Combined Read tab and forwarded
     by the Brain. It is a SUMMARY of conclusions the detector's own
     engines already reached — structure, the deciding level, pressure,
     the reversal lifecycle, participation, risk — not raw data for the
     model to re-interpret. That distinction is the point: if Haiku were
     given the raw engines it would form its own view, and two screens
     would describe one market differently.

     It arrives from the browser, so it is treated as untrusted input:
     shape-checked, size-capped, and dropped entirely if stale. A stale
     read is worse than none — it describes a market that has moved. */
  let confluenceBlock = null;
  try {
    const cf = confluence && typeof confluence === 'object' ? confluence : null;
    if (cf) {
      const ageMs = Number(cf.ageMs);
      const fresh = Number.isFinite(ageMs) && ageMs >= 0 && ageMs < 5 * 60 * 1000;
      if (fresh) {
        const trimmed = JSON.stringify(cf).slice(0, 6000);
        confluenceBlock = JSON.parse(trimmed.length === JSON.stringify(cf).length ? trimmed : JSON.stringify({
          state: cf.state, word: cf.word, line: cf.line, trend: cf.trend,
          decides: cf.decides, pressure: cf.pressure, reversal: cf.reversal,
          trigger: cf.trigger, participation: cf.participation, risk: cf.risk,
          supports: cf.supports, against: cf.against, suspended: cf.suspended, feed: cf.feed
        }));
      } else {
        console.log('[ANALYSE] combined read dropped — stale or undated');
      }
    }
  } catch (e) { confluenceBlock = null; }
  ...
  const prompt = [
    ...
    ...(confluenceBlock ? [
      '\n=== SOURCE 6: Combined Read (Pattern Detector) ===',
      JSON.stringify(confluenceBlock, null, 2)
    ] : []),
...
6. Combined Read — present only sometimes. Conclusions already reached by the Pattern Detector's own structure, pressure, reversal, participation and risk engines. Read the rules below.

=== SOURCE 6 COMBINED READ RULES — follow exactly ===
1. AUTHORITY ON STRUCTURE. If source 6 is present, its "trend", "decides" (the deciding level) and "reversal" are the platform's official reading. Do NOT contradict them from sources 1-5. If another source disagrees, say so as a disagreement, never as a correction.
2. EXPLAIN, DO NOT RE-DECIDE. Your job with this source is to connect it to the others and put it in plain words. It has already weighed its evidence.
3. "suspended": true means the platform has withheld the right to act on the reading. Report the readings; do not build a trade setup on them.
4. "feed" tells you whether data was missed. If feed.ok is false or feed.gapBars > 0, say plainly that the picture may be incomplete.
5. NOT A FORECAST. This source sorts evidence; it has no measured predictive edge. Never present its state as a prediction of what price will do — describe the situation, and let the trader decide.
6. If source 6 is absent, analyse sources 1-5 as normal and do not mention it.
=== END SOURCE 6 RULES ===
...
async function fetchNews() {
  ...
  /* ── SECOND CALENDAR ─────────────────────────────────────────────
     FairEconomy carries ForexFactory's list, and the two calendars
     genuinely differ. Measured on 24 Sep 2026: FF had ONE row,
     "Unemployment Claims" at 12:30, where MyFxBook had three separate
     claims releases — and FF carried no New Home Sales at 14:00 at all.
     So the second source is merged in here: the same event kept once,
     events only one side has brought in.
     Additive and non-fatal by design. If the fetch or the parse fails,
     `merged` is already a complete ForexFactory list and goes out
     unchanged — a broken scraper must never take the news tab down. */
  let finalEvents = merged;
  try {
    const extra = await fetchMyfxbookCalendar();
    if (extra && extra.length) {
      const out = mergeCalendars(merged, extra);
      if (out.length >= merged.length) {           // never fewer than FF alone
        finalEvents = out;
        console.log(`[NEWS] MyFxBook: ${extra.length} parsed, ${out.length - merged.length} not in ForexFactory`);
      }
    }
  } catch (e) {
    console.warn('[NEWS] MyFxBook merge skipped:', e.message);
  }

  newsEvents = finalEvents;
  broadcast(null, 'NEWS_UPDATE', finalEvents);
  console.log(`Calendar: ${finalEvents.length} HIGH impact events across both weeks`);
}

/* ══════════════════════════════════════════════════════════════════
   MYFXBOOK — the second calendar

   They publish no calendar API, so this reads their page. Three rules
   follow from that:
     - it runs ONCE an hour, cached, so one fetch serves every customer
       rather than one per request
     - every failure is swallowed and logged; the caller keeps its
       ForexFactory list rather than losing the tab
     - a row is kept only if currency, time and title can all be read.
       Nothing is guessed — a countdown to a time we invented would be
       worse than no row at all.
   ══════════════════════════════════════════════════════════════════ */
const MFB_URL = 'https://www.myfxbook.com/forex-economic-calendar';
const MFB_TTL_MS = 60 * 60 * 1000;
let mfbCache = { at: 0, rows: [] };

function mfbNum(t) {
  const s = String(t || '').trim();
  if (!s || s === '-') return null;
  return /^-?[\d.,]+\s*[KMB%]?$/.test(s) ? s : null;
}

/* Two ways to read the time, both tried. Their rows usually carry a unix
   timestamp in a data attribute; when they do not, the visible date and
   time are parsed. Relying on either alone is how a scraper silently
   returns nothing after a redesign. */
function mfbParseRows(html) {
  const rows = [];
  const CCY = /\b(USD|EUR|GBP|JPY|CHF|AUD|CAD|NZD)\b/;
  const MO = { Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11 };
  const trs = String(html || '').split(/<tr\b/i).slice(1);
  ...
  for (const tr of trs) {
    ...
    if (!/high/i.test(line) && !/impact-high|sprite-high/i.test(tr)) continue;

    const c = CCY.exec(line);
    if (!c) continue;

    let ts = null;
    const attr = /data-(?:timestamp|event-date|date)="(\d{9,13})"/i.exec(tr);
    if (attr) {
      const n = Number(attr[1]);
      /* seconds or milliseconds — the same trap that made the Live badge
         read "20605 days ago" on the EA feed. */
      ts = n > 1e12 ? Math.round(n / 1000) : n;
    }
    if (ts == null) {
      const d = /\b([A-Z][a-z]{2})\s+(\d{1,2})\b/.exec(line);
      const t = /\b(\d{1,2}):(\d{2})\b/.exec(line);
      if (!d || !t) continue;
      const mo = MO[d[1]];
      if (mo == null) continue;
      ts = Math.round(Date.UTC(new Date().getUTCFullYear(), mo, +d[2], +t[1], +t[2]) / 1000);
    }
    if (!Number.isFinite(ts)) continue;

    /* The date cell ("Sep 24") is six characters and survived the old
       length test, so it was being picked as the title and every merge
       then failed to match. A title must contain a letter run of three
       or more AND not look like a date, a time, a figure or an impact
       label. */
    const title = cells.find(x =>
      x.length > 5 &&
      /[A-Za-z]{3}/.test(x) &&
      !CCY.test(x) &&
      !/^[A-Z][a-z]{2}\s+\d{1,2}$/.test(x) &&
      !/^\d{1,2}:\d{2}$/.test(x) &&
      !/^-?[\d.,]+\s*[KMB%]?$/.test(x) &&
      !/^(high|medium|low)$/i.test(x));
    if (!title) continue;

    const figs = cells.map(mfbNum).filter(v => v !== null);
    rows.push({
      title, country: c[1], impact: 'high', timestamp: ts,
      actual:   figs.length > 0 ? figs[0] : null,
      forecast: figs.length > 1 ? figs[1] : null,
      previous: figs.length > 2 ? figs[2] : null
    });
  }
  return rows;
}

async function fetchMyfxbookCalendar() {
  if (Date.now() - mfbCache.at < MFB_TTL_MS) return mfbCache.rows;
  try {
    const r = await axios.get(MFB_URL, {
      timeout: 12000,
      headers: {
    ...
    const rows = mfbParseRows(r.data);
    /* An empty parse almost always means their markup moved, not that
       the world has no news. Keep the last good rows and say so. */
    if (!rows.length) {
      console.warn('[NEWS] MyFxBook: no parseable HIGH rows — markup may have changed');
      return mfbCache.rows;
    }
    mfbCache = { at: Date.now(), rows };
    return rows;
  } catch (e) {
    console.warn('[NEWS] MyFxBook fetch failed:', e.message);
    return mfbCache.rows;                          // last good list, or []
  }
}

/* ── THE MERGE ────────────────────────────────────────────────────
   Same event kept once; anything only the second source has is added.
   Currency plus time is the primary key — two CHF items at 07:30 are
   genuinely two events, so time alone can never decide a match and the
   titles must agree as well. */
function newsNormTitle(t) {
  return String(t || '').toLowerCase()
    .replace(/\([^)]*\)/g, ' ')
    .replace(/\b(m\/m|q\/q|y\/y|mom|qoq|yoy)\b/g, ' ')
    .replace(/[^a-z0-9 ]/g, ' ')
    .replace(/\b(the|of|for|and|rate|index|change|final|prelim|advance)\b/g, ' ')
    .replace(/\s+/g, ' ').trim();
}

/* Explicit and readable on purpose. A fuzzy scorer that quietly merged
   "Core CPI" with "CPI" would be worse than no matcher at all — and a
   human can correct this table when a name changes. */
const NEWS_ALIASES = [
  ['unemployment claims', 'initial jobless claims'],
  ['unemployment claims', 'jobless claims'],
  ['non farm employment', 'non farm payrolls'],
  ['non farm employment', 'nonfarm payrolls'],
  ['federal funds', 'fed interest decision'],
  ['federal funds', 'fomc interest decision'],
  ['snb policy', 'snb interest decision'],
  ['official cash', 'rbnz interest decision']
];
function newsAliasKey(n) {
  for (const pair of NEWS_ALIASES) {
    if (n.indexOf(pair[0]) >= 0 || n.indexOf(pair[1]) >= 0) return 'alias:' + pair[0];
  }
  return null;
}
...
const NEWS_SAME_MIN_S = 6 * 60;
function newsSameEvent(x, y) {
  if (x.country !== y.country) return false;
  if (Math.abs(x.timestamp - y.timestamp) > NEWS_SAME_MIN_S) return false;
  const nx = newsNormTitle(x.title), ny = newsNormTitle(y.title);
  if (nx === ny) return true;
  const ax = newsAliasKey(nx), ay = newsAliasKey(ny);
  if (ax && ay && ax === ay) return true;
  return newsOverlap(nx, ny) >= 0.7;
}

function mergeCalendars(primary, second) {
  const out = (primary || []).map(e => Object.assign({}, e, { sources: ['forexfactory'] }));
  for (const b of (second || [])) {
    const hit = out.find(a => newsSameEvent(a, b));
    if (hit) {
      hit.sources = Array.from(new Set((hit.sources || []).concat('myfxbook')));
      hit.rolledUp = (hit.rolledUp || 1) + 1;
      /* Prefer the HEADLINE member's figures. Without this the last row
         to match wins, which put the 4-week Average's number on a row
         labelled Unemployment Claims. */
      const bHead = /^initial jobless|^unemployment claims/.test(newsNormTitle(b.title));
      if (bHead || hit.forecast == null) {
        if (b.forecast != null) hit.forecast = b.forecast;
        if (b.actual   != null) hit.actual   = b.actual;
      }
      if (hit.previous == null && b.previous != null) hit.previous = b.previous;
      continue;
    }
    out.push(Object.assign({}, b, { sources: ['myfxbook'] }));
  }
...
supportRoute.start();   // nightly attachment clean-up, 00:40 UTC
