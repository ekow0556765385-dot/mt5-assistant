...
app.post('/api/analyse', async (req, res) => {
  ...
  if (smcContext && smcContext.sweepStatus) {
    ...
    const entryLine  = sweepState === 'confirmed'
      ? 'YES — sweep complete, close back inside OB confirmed. Focus entirely on entry, SL and TP. Do NOT mention the sweep warning anymore.'
      : sweepState === 'swept'
        ? 'NOT YET — wick through OB but no close back inside yet. Hold off entry.'
        : 'NO — sweep has not happened. Do not enter.';
