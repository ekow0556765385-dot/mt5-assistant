...
// ── Auth middleware ────────────────────────────────────────────
const { requirePlan, requireFramedByDashboard, requireAuth, getMe, validateKey, regenerateKey, setBrokerType, cancelSubscription, issueTicketRoute, isOwner, getUserCredits, deductCredits, resetUserCredits, MONTHLY_CREDIT_USD, SUPABASE_URL, supabaseServiceHeaders, verifyAppSession, getUserIdForLicenceKey, getSourceForLicenceKey } = require('./auth-middleware');
...
app.get('/brain',      requireFramedByDashboard, requirePlan('pro'), (req, res) => res.sendFile(path.join(__dirname, 'mt5_trading_brain_v4.html')));
...
const BW_ALL_PAIRS  = ['EURUSD','GBPUSD','USDJPY','AUDUSD','USDCAD','XAUUSD'];
...
app.get('/api/watch-pairs', requireAuth, async (req, res) => {
  try {
    ...
    const pairs = Array.isArray(row.watch_pairs) && row.watch_pairs.length
      ? row.watch_pairs.slice(0, limit)
...
// ── POST /api/risk-alert ──────────────────────────────────────────
// Risk Radar escalation notice. The dashboard only calls this when a pair
// crosses INTO Elevated or Stand down — never on every recompute — so the
// per-user rate limit below is a backstop against a client bug, not the
// primary throttle.
//
// requireAuth, not requirePlan('pro'): the same reasoning as the Telegram
// confluence alerts above. A trader being told the market is being pushed
// around is a safety message, not a paid feature.
const RISK_LEVELS   = ['elevated', 'standdown'];
const riskAlertLast = {};                 // `${userId}::${symbol}` -> { level, at }
const RISK_COOLDOWN_MS = 10 * 60 * 1000;  // one notice per pair per 10 min

app.post('/api/risk-alert', requireAuth, async (req, res) => {
  try {
    const { symbol, level, score, factors } = req.body || {};
    if (!symbol || !RISK_LEVELS.includes(String(level))) {
      return res.status(400).json({ error: 'Need a symbol and level of elevated|standdown' });
    }
    const n = Math.max(0, Math.min(100, parseInt(score, 10) || 0));

    // Same normaliser used everywhere else — the dashboard sends a BARE pair
    // (EURUSD) while the EA store keys are broker symbols (EURUSDc).
    const sym = normalisePair(String(symbol)) || String(symbol).toUpperCase();
    const key = `${req.user.id}::${sym}`;
    const prev = riskAlertLast[key];

    // Suppress a repeat of the SAME level inside the cooldown. An escalation
    // from elevated -> standdown always goes out: that is new information.
    if (prev && prev.level === level && (Date.now() - prev.at) < RISK_COOLDOWN_MS) {
      return res.json({ ok: true, sent: false, reason: 'cooldown' });
    }
    riskAlertLast[key] = { level, at: Date.now() };

    // Respect the free tier's pair selection, exactly like the confluence
    // alerts do — no point warning someone about a pair they don't watch.
    try {
      const { data } = await axios.get(
        `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${req.user.id}&select=watch_pairs,plan`,
        { headers: supabaseServiceHeaders() }
      ...
      const row = (data && data[0]) || {};
      const watched = Array.isArray(row.watch_pairs) ? row.watch_pairs : [];
      if (row.plan === 'free' && watched.length &&
          !watched.some(w => normalisePair(String(w)) === sym)) {
        return res.json({ ok: true, sent: false, reason: 'not in watch list' });
      }
    } catch (e) {
      // Column missing or Supabase unreachable — send anyway rather than
      // silently swallowing a warning.
      console.warn('[RISK-ALERT] watch_pairs lookup skipped:', e.message);
    }

    const isStand = level === 'standdown';
    const lines = (Array.isArray(factors) ? factors : []).slice(0, 4)
      .map(f => `• ${String(f && f.label || '').slice(0, 60)}${f && f.weight ? ` (+${parseInt(f.weight, 10) || 0})` : ''}`)
      .join('\n');

    const msg =
      `${isStand ? '🛑' : '⚠️'} <b>MARKET INTEGRITY ${isStand ? 'STAND DOWN' : 'ELEVATED'}</b>\n` +
      `📊 <b>${sym}</b> · risk score <b>${n}/100</b>\n\n` +
      (lines ? `${lines}\n\n` : '') +
      (isStand
        ? `⚡ Conditions associated with erratic price behaviour. Avoid fresh entries until this clears.`
        : `⚡ Several risk conditions are stacked. Reduce size and widen stops.`);

    sendTelegramToUser(req.user.id, msg);
    console.log(`[RISK-ALERT] user=${req.user.id} ${sym} ${level} score=${n}`);
    res.json({ ok: true, sent: true });
  } catch (err) {
    console.error('[RISK-ALERT ERROR]', err.message);
    res.status(500).json({ error: 'Could not send risk alert' });
  }
});
