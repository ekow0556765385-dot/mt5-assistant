...
function computeR(trade) {
  try {
    ...
    const lots   = parseFloat(trade.lots        || 0.01);
    const type   = (trade.type || trade.action  || '').toUpperCase();

    // Primary: price-based R (most accurate — uses actual SL distance)
    if (sl && open && close) {
    ...
    // Fallback A: use pips if available
    if (trade.pips !== undefined && trade.pips !== null && trade.pips !== 0) {
      // Estimate SL as typical 30 pip risk if no SL stored
      const assumedRiskPips = 30;
      return parseFloat((parseFloat(trade.pips) / assumedRiskPips).toFixed(3));
    }

    // Fallback B: dollar profit / assumed 1% risk
    // Can't compute true R without knowing risk amount — return null
    return null;
  } catch(e) { return null; }
}
...
app.post('/api/math-trades', (req, res) => {
  try {
    ...
    const incoming  = (body.closed_trades || []).map(t => ({
      ...t,
      // Trust r_multiple sent by EA — only compute server-side if EA didn't send it
      // Also honour r_method label sent by EA (price/dollar/pip_estimate)
      r_multiple: (t.r_multiple !== undefined && t.r_multiple !== null && t.r_multiple !== 0)
  ...
  try {
    ...
    if ((!math.closed_trades || math.closed_trades.length === 0) && s.closedTrades && s.closedTrades.length > 0) {
      const mapped = s.closedTrades.map((t, i) => ({
        ...
        r_multiple:  (t.r_multiple !== undefined && t.r_multiple !== null && t.r_multiple !== 0)
                        ? parseFloat(t.r_multiple)
                        : computeR({ open_price: t.openPrice||t.open_price, close_price: t.closePrice||t.close_price, sl: t.sl, type: t.type })
      }));
    ...
    try {
      ...
      if (fs.existsSync(sessionFile)) {
        ...
        // Pull executed_orders from agent session
        // NOTE: agent orders only have entry price + profit, NOT close price
        // Close price is only available from MathReporter EA reading deal history
        // We merge these with any existing math_trades data — MathReporter data takes priority
        if (session.executed_orders && session.executed_orders.length > 0) {
          const agentOrders = session.executed_orders.filter(o => o.profit !== undefined);

          if (!math.closed_trades || math.closed_trades.length === 0) {
            // No MathReporter data yet — show agent orders with what we have
            // open_price = entry when agent placed order (correct)
            // close_price = unknown (agent never records it) — show as null
            math.closed_trades = agentOrders.map((o, i) => {
              // Try to get close_price from various places the agent might store it
              const close_price = o.close_price || o.exit_price || o.closePrice || null;
              // Entry price: agent stores as 'price' (bid at decision time) or open_price
              const open_price  = o.open_price  || o.entry_price || o.price || null;
              const sl          = o.sl  || o.stop_loss  || 0;
              const type        = (o.action || o.type || '').toUpperCase();
              const profit      = parseFloat(o.profit || 0);
              const r_multiple  = (o.r_multiple != null && o.r_multiple !== 0)
                                  ? parseFloat(o.r_multiple)
                                  : computeR({ open_price, close_price, sl, type, profit });
              return {
                ticket:      o.ticket || o.magic || (i + 1),
                symbol:      o.symbol || '—',
                type,
                lots:        o.lots   || o.volume || 0,
                open_price,
                close_price,
                sl,
                tp:          o.tp || 0,
                profit,
                pips:        o.pips || null,
                close_time:  o.close_time || o.closed_at || new Date().toISOString(),
                r_multiple,
                r_method:    r_multiple !== null ? 'dollar' : 'unknown',
                _source:     'agent_session'
              };
            });
            math.stats = recalcMathStats(math.closed_trades);
          } else {
            // MathReporter data exists — try to enrich it with profit from agent session
            // (in case MathReporter sent before trade was fully settled)
            const agentByTicket = {};
            agentOrders.forEach(o => {
              if (o.ticket || o.magic) agentByTicket[o.ticket || o.magic] = o;
            });
            math.closed_trades = math.closed_trades.map(t => {
              const match = agentByTicket[t.ticket];
              if (match && match.profit && !t.profit) {
                return { ...t, profit: parseFloat(match.profit) };
              }
              return t;
            });
          }
